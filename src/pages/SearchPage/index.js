import React from "react";

import { Column } from "components/Column";
import { Row } from "components/Row";
import { Text } from "components/Text";
import { Stack } from "components/Stack";
import { Image } from "components/Image";
import { Button } from "components/Button";
import { Line } from "components/Line";
import { Switch } from "components/Switch";
import { CheckBox } from "components/CheckBox";
import { List } from "components/List";
import { RatingBar } from "components/RatingBar";
import { Input } from "components/Input";

const SearchPagePage = () => {
  return (
    <>
      <Column
        className="bg-gray_50 mx-[auto] w-[100%]"
        compId="870:781"
        comWidth={1920}
        comHeight={3489}
        compLeft={0}
        compRight={0}
        compType="Column"
      >
        <Column
          className="items-center justify-start w-[100%]"
          compId="1333"
          comWidth={1920}
          comHeight={3489}
          compLeft={0}
          compRight={0}
          compType="Column"
        >
          <header
            className="font-poppins w-[100%]"
            compId="304"
            comWidth={1920}
            comHeight={120}
            compLeft={0}
            compRight={0}
            compType="Header"
          >
            <Row
              className="items-center justify-start rounded-bl-[0] rounded-br-[8px] rounded-tl-[0] rounded-tr-[8px] w-[100%]"
              compId="1083:218"
              comWidth={1920}
              comHeight={120}
              compLeft={0}
              compRight={0}
              compType="Row"
            >
              <Text
                className="font-semibold xl:ml-[106px] 2xl:ml-[120px] 3xl:ml-[144px] ml-[160px] lg:ml-[93px] lg:my-[19px] xl:my-[22px] 2xl:my-[24px] 3xl:my-[29px] my-[33px] text-deep_orange_400 lg:text-fs21 xl:text-fs24 2xl:text-fs27 3xl:text-fs32 text-fs36 text-left w-[6%]"
                compId="1083:199"
                comWidth={112}
                comHeight={54}
                compLeft={160}
                compRight={0}
                compType="Text"
              >{`Tronix`}</Text>
              <Row
                className="items-center justify-center lg:mb-[27px] xl:mb-[31px] 2xl:mb-[35px] 3xl:mb-[42px] mb-[47px] lg:ml-[263px] xl:ml-[301px] 2xl:ml-[339px] 3xl:ml-[407px] ml-[452px] lg:mt-[26px] xl:mt-[30px] 2xl:mt-[34px] 3xl:mt-[41px] mt-[46px] w-[25%]"
                compId="1351"
                comWidth={473}
                comHeight={27}
                compLeft={452}
                compRight={0}
                compType="Row"
              >
                <Text
                  className="cursor-pointer hover:font-normal font-normal font-poppins not-italic lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_800 text-left w-[11%]"
                  compId="1083:219"
                  comWidth={54}
                  comHeight={27}
                  compLeft={0}
                  compRight={0}
                  compType="Text"
                >{`Home`}</Text>
                <Text
                  className="cursor-pointer hover:font-normal font-normal font-poppins lg:ml-[28px] xl:ml-[32px] 2xl:ml-[36px] 3xl:ml-[43px] ml-[48px] not-italic lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_800 text-left w-[11%]"
                  compId="1083:220"
                  comWidth={54}
                  comHeight={27}
                  compLeft={48}
                  compRight={0}
                  compType="Text"
                >{`About`}</Text>
                <Text
                  className="cursor-pointer hover:font-normal font-normal font-poppins lg:ml-[28px] xl:ml-[32px] 2xl:ml-[36px] 3xl:ml-[43px] ml-[48px] not-italic lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_800 text-left w-[15%]"
                  compId="1083:221"
                  comWidth={70}
                  comHeight={27}
                  compLeft={48}
                  compRight={0}
                  compType="Text"
                >{`Product`}</Text>
                <Text
                  className="cursor-pointer hover:font-normal font-normal font-poppins lg:ml-[28px] xl:ml-[32px] 2xl:ml-[36px] 3xl:ml-[43px] ml-[48px] not-italic lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_800 text-left w-[8%]"
                  compId="1083:222"
                  comWidth={40}
                  comHeight={27}
                  compLeft={48}
                  compRight={0}
                  compType="Text"
                >{`Blog`}</Text>
                <Text
                  className="cursor-pointer hover:font-normal font-normal font-nunitosans lg:ml-[28px] xl:ml-[32px] 2xl:ml-[36px] 3xl:ml-[43px] ml-[48px] my-[1px] not-italic lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_800 text-left w-[13%]"
                  compId="1083:223"
                  comWidth={63}
                  comHeight={25}
                  compLeft={48}
                  compRight={0}
                  compType="Text"
                >{`Contact`}</Text>
              </Row>
              <Row
                className="items-end justify-center lg:mb-[25px] xl:mb-[29px] 2xl:mb-[33px] 3xl:mb-[39px] mb-[44px] lg:ml-[235px] xl:ml-[268px] 2xl:ml-[302px] 3xl:ml-[362px] ml-[403px] lg:mt-[18px] xl:mt-[21px] 2xl:mt-[24px] 3xl:mt-[28px] mt-[32px] w-[8%]"
                compId="1283"
                comWidth={160}
                comHeight={44}
                compLeft={403}
                compRight={0}
                compType="Row"
              >
                <Stack
                  className="lg:h-[26px] xl:h-[30px] 2xl:h-[34px] 3xl:h-[40px] h-[44px] lg:w-[25px] xl:w-[29px] 2xl:w-[33px] 3xl:w-[39px] w-[44px]"
                  compId="1335"
                  comWidth={44}
                  comHeight={44}
                  compLeft={0}
                  compRight={0}
                  compType="Stack"
                >
                  <Image
                    src={"images/img_shoppingbag_6.svg"}
                    className="absolute bottom-[0] lg:h-[19px] xl:h-[22px] 2xl:h-[25px] 3xl:h-[29px] h-[32px] left-[0] object-contain lg:w-[18px] xl:w-[21px] 2xl:w-[24px] 3xl:w-[28px] w-[32px]"
                    compId="1083:210"
                    comWidth={32}
                    comHeight={32}
                    compLeft={0}
                    compRight={0}
                    compType="Image"
                    alt="ShoppingBag"
                  />
                  <Column
                    className="absolute bg-deep_orange_400 lg:h-[14px] xl:h-[17px] 2xl:h-[19px] 3xl:h-[22px] h-[24px] items-center justify-start lg:py-[1px] 3xl:py-[2px] 2xl:py-[2px] xl:py-[2px] py-[3px] right-[0] rounded-radius50 top-[0] lg:w-[14px] xl:w-[16px] 2xl:w-[18px] 3xl:w-[21px] w-[24px]"
                    compId="303"
                    comWidth={24}
                    comHeight={24}
                    compLeft={0}
                    compRight={0}
                    compType="Column"
                  >
                    <Text
                      className="font-normal mx-[auto] not-italic 3xl:text-fs10 text-fs12 lg:text-fs7 xl:text-fs8 2xl:text-fs9 text-gray_50 text-left w-[67%]"
                      compId="1083:217"
                      comWidth={16}
                      comHeight={18}
                      compLeft={4}
                      compRight={4}
                      compType="Text"
                    >{`99`}</Text>
                  </Column>
                </Stack>
                <Image
                  src={"images/img_email_9.svg"}
                  className="lg:h-[19px] xl:h-[22px] 2xl:h-[25px] 3xl:h-[29px] h-[32px] lg:ml-[11px] xl:ml-[13px] 2xl:ml-[15px] 3xl:ml-[18px] ml-[20px] 3xl:mt-[10px] mt-[12px] lg:mt-[7px] xl:mt-[8px] 2xl:mt-[9px] object-contain lg:w-[18px] xl:w-[21px] 2xl:w-[24px] 3xl:w-[28px] w-[32px]"
                  compId="1083:212"
                  comWidth={32}
                  comHeight={32}
                  compLeft={20}
                  compRight={0}
                  compType="Image"
                  alt="Email"
                />
                <Image
                  src={"images/img_menu.svg"}
                  className="lg:h-[19px] xl:h-[22px] 2xl:h-[25px] 3xl:h-[29px] h-[32px] lg:ml-[18px] xl:ml-[21px] 2xl:ml-[24px] 3xl:ml-[28px] ml-[32px] 3xl:mt-[10px] mt-[12px] lg:mt-[7px] xl:mt-[8px] 2xl:mt-[9px] object-contain lg:w-[18px] xl:w-[21px] 2xl:w-[24px] 3xl:w-[28px] w-[32px]"
                  compId="1083:214"
                  comWidth={32}
                  comHeight={32}
                  compLeft={32}
                  compRight={0}
                  compType="Image"
                  alt="Menu"
                />
              </Row>
            </Row>
          </header>
          <Text
            className="font-bold font-montserrat lg:mt-[46px] xl:mt-[53px] 2xl:mt-[60px] 3xl:mt-[72px] mt-[80px] mx-[auto] lg:text-fs28 xl:text-fs32 2xl:text-fs36 3xl:text-fs43 text-fs48 text-gray_800 text-left w-[34%]"
            compId="1083:410"
            comWidth={657}
            comHeight={59}
            compLeft={10}
            compRight={10}
            compType="Text"
          >{`What are you looking for?`}</Text>
          <Row
            className="font-poppins items-center justify-center lg:mt-[28px] xl:mt-[32px] 2xl:mt-[36px] 3xl:mt-[43px] mt-[48px] mx-[auto] w-[39%]"
            compId="1083:526"
            comWidth={740}
            comHeight={60}
            compLeft={10}
            compRight={10}
            compType="Row"
          >
            <Image
              src={"images/img_search_2.svg"}
              className="lg:h-[35px] xl:h-[41px] 2xl:h-[46px] 3xl:h-[55px] h-[60px] object-contain rounded-radius8 lg:w-[35px] xl:w-[40px] 2xl:w-[45px] 3xl:w-[54px] w-[60px]"
              compId="889:637"
              comWidth={60}
              comHeight={60}
              compLeft={0}
              compRight={0}
              compType="Image"
              alt="search"
            />
            <Stack
              className="lg:h-[35px] xl:h-[41px] 2xl:h-[46px] 3xl:h-[55px] h-[60px] lg:ml-[11px] xl:ml-[13px] 2xl:ml-[15px] 3xl:ml-[18px] ml-[20px] shadow-bs2 w-[89%]"
              compId="1337"
              comWidth={660}
              comHeight={60}
              compLeft={20}
              compRight={0}
              compType="Stack"
            >
              <Button
                className="absolute bg-gray_50 font-normal inset-y-[0] left-[0] not-italic xl:pb-[12px] 3xl:pb-[16px] pb-[18.545px] xl:pt-[11px] 3xl:pt-[15px] pt-[17.545px] lg:py-[10px] 2xl:py-[13px] right-[2%] rounded-radius8 text-center lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_500 w-[98%]"
                compId="302"
                comWidth={648}
                comHeight={60}
                compLeft={0}
                compRight={12}
                compType="Button"
              >{`Search item here`}</Button>
              <Image
                src={"images/img_search_3.svg"}
                className="absolute lg:h-[35px] xl:h-[41px] 2xl:h-[46px] 3xl:h-[55px] h-[60px] object-contain right-[0] rounded-bl-[0] rounded-br-[8px] rounded-tl-[0] rounded-tr-[8px] lg:w-[35px] xl:w-[40px] 2xl:w-[45px] 3xl:w-[54px] w-[60px]"
                compId="889:631"
                comWidth={60}
                comHeight={60}
                compLeft={0}
                compRight={0}
                compType="Image"
                alt="search"
              />
            </Stack>
          </Row>
          <Row
            className="font-poppins items-start justify-start lg:mt-[29px] xl:mt-[34px] 2xl:mt-[38px] 3xl:mt-[45px] mt-[51px] w-[100%]"
            compId="1332"
            comWidth={1920}
            comHeight={2081}
            compLeft={0}
            compRight={0}
            compType="Row"
          >
            <Column
              className="border border-gray_500 border-solid items-start justify-start xl:ml-[106px] 2xl:ml-[120px] 3xl:ml-[144px] ml-[160px] lg:ml-[93px] lg:mt-[116px] xl:mt-[133px] 2xl:mt-[150px] 3xl:mt-[180px] mt-[200px] lg:py-[18px] xl:py-[21px] 2xl:py-[24px] 3xl:py-[28px] py-[32px] rounded-radius16 w-[20%]"
              compId="1328"
              comWidth={376}
              comHeight={1881}
              compLeft={160}
              compRight={0}
              compType="Column"
            >
              <Column
                className="items-start justify-start w-[100%]"
                compId="1289"
                comWidth={376}
                comHeight={516}
                compLeft={0}
                compRight={0}
                compType="Column"
              >
                <Text
                  className="font-bold lg:mx-[18px] xl:mx-[21px] 2xl:mx-[24px] 3xl:mx-[28px] mx-[32px] lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_800 text-left w-[35%]"
                  compId="1089:266"
                  comWidth={130}
                  comHeight={27}
                  compLeft={32}
                  compRight={32}
                  compType="Text"
                >{`All Categories`}</Text>
                <Column
                  className="lg:mt-[18px] xl:mt-[21px] 2xl:mt-[24px] 3xl:mt-[28px] mt-[32px] w-[100%]"
                  compId="1355"
                  comWidth={376}
                  comHeight={457}
                  compLeft={0}
                  compRight={0}
                  compType="Column"
                >
                  <Row
                    className="items-center justify-between w-[100%]"
                    compId="1288"
                    comWidth={376}
                    comHeight={457}
                    compLeft={0}
                    compRight={0}
                    compType="Row"
                  >
                    <Column
                      className="items-start lg:ml-[18px] xl:ml-[21px] 2xl:ml-[24px] 3xl:ml-[28px] ml-[32px] w-[61%]"
                      compId="1089:267"
                      comWidth={231}
                      comHeight={457}
                      compLeft={32}
                      compRight={0}
                      compType="Column"
                    >
                      <Text
                        className="font-normal mr-[10px] lg:mr-[5px] xl:mr-[6px] 2xl:mr-[7px] 3xl:mr-[9px] not-italic lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_800 text-left w-[96%]"
                        compId="1089:268"
                        comWidth={221}
                        comHeight={27}
                        compLeft={0}
                        compRight={10}
                        compType="Text"
                      >
                        <span className="text-gray_800 font-poppins">
                          <>{`Computer & Laptop `}</>
                        </span>
                        <span className="text-gray_500 font-poppins">
                          <>{`(25)`}</>
                        </span>
                      </Text>
                      <Text
                        className="font-normal mr-[10px] lg:mr-[5px] xl:mr-[6px] 2xl:mr-[7px] 3xl:mr-[9px] xl:mt-[10px] 2xl:mt-[12px] 3xl:mt-[14px] mt-[16px] lg:mt-[9px] not-italic lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_800 text-left w-[81%]"
                        compId="1089:269"
                        comWidth={187}
                        comHeight={27}
                        compLeft={0}
                        compRight={10}
                        compType="Text"
                      >
                        <span className="text-gray_800 font-poppins">
                          <>
                            {`Mobile & Tablet`}
                            <br />
                            {``}
                          </>
                        </span>
                        <span className="text-gray_500 font-poppins">
                          <>{`(125)`}</>
                        </span>
                      </Text>
                      <Text
                        className="font-normal mr-[10px] lg:mr-[5px] xl:mr-[6px] 2xl:mr-[7px] 3xl:mr-[9px] xl:mt-[10px] 2xl:mt-[12px] 3xl:mt-[14px] mt-[16px] lg:mt-[9px] not-italic lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_800 text-left w-[56%]"
                        compId="1089:270"
                        comWidth={129}
                        comHeight={27}
                        compLeft={0}
                        compRight={10}
                        compType="Text"
                      >
                        <span className="text-gray_800 font-poppins">
                          <>
                            {`Camera`}
                            <br />
                            {` `}
                          </>
                        </span>
                        <span className="text-gray_500 font-poppins">
                          <>{`(150)`}</>
                        </span>
                      </Text>
                      <Text
                        className="font-normal mr-[10px] lg:mr-[5px] xl:mr-[6px] 2xl:mr-[7px] 3xl:mr-[9px] xl:mt-[10px] 2xl:mt-[12px] 3xl:mt-[14px] mt-[16px] lg:mt-[9px] not-italic lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_800 text-left w-[77%]"
                        compId="1089:271"
                        comWidth={178}
                        comHeight={27}
                        compLeft={0}
                        compRight={10}
                        compType="Text"
                      >
                        <span className="text-gray_800 font-poppins">
                          <>
                            {`TV & Smart Box`}
                            <br />
                            {``}
                          </>
                        </span>
                        <span className="text-gray_500 font-poppins">
                          <>{`(75)`}</>
                        </span>
                      </Text>
                      <Text
                        className="font-normal mr-[10px] lg:mr-[5px] xl:mr-[6px] 2xl:mr-[7px] 3xl:mr-[9px] xl:mt-[10px] 2xl:mt-[12px] 3xl:mt-[14px] mt-[16px] lg:mt-[9px] not-italic lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_800 text-left w-[85%]"
                        compId="1089:272"
                        comWidth={197}
                        comHeight={27}
                        compLeft={0}
                        compRight={10}
                        compType="Text"
                      >
                        <span className="text-gray_800 font-poppins">
                          <>
                            {`Home Appliance`}
                            <br />
                            {` `}
                          </>
                        </span>
                        <span className="text-gray_500 font-poppins">
                          <>{`(75)`}</>
                        </span>
                      </Text>
                      <Text
                        className="font-normal mr-[10px] lg:mr-[5px] xl:mr-[6px] 2xl:mr-[7px] 3xl:mr-[9px] xl:mt-[10px] 2xl:mt-[12px] 3xl:mt-[14px] mt-[16px] lg:mt-[9px] not-italic lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_800 text-left w-[70%]"
                        compId="1089:273"
                        comWidth={162}
                        comHeight={27}
                        compLeft={0}
                        compRight={10}
                        compType="Text"
                      >
                        <span className="text-gray_800 font-poppins">
                          <>{`Smart Watch `}</>
                        </span>
                        <span className="text-gray_500 font-poppins">
                          <>{`(45)`}</>
                        </span>
                      </Text>
                      <Text
                        className="font-normal xl:mt-[10px] 2xl:mt-[12px] 3xl:mt-[14px] mt-[16px] lg:mt-[9px] not-italic lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_800 text-left w-[100%]"
                        compId="1089:274"
                        comWidth={231}
                        comHeight={27}
                        compLeft={0}
                        compRight={0}
                        compType="Text"
                      >
                        <span className="text-gray_800 font-poppins">
                          <>
                            {`Microphone & Audio`}
                            <br />
                            {` `}
                          </>
                        </span>
                        <span className="text-gray_500 font-poppins">
                          <>{`(55)`}</>
                        </span>
                      </Text>
                      <Text
                        className="font-normal mr-[10px] lg:mr-[5px] xl:mr-[6px] 2xl:mr-[7px] 3xl:mr-[9px] xl:mt-[10px] 2xl:mt-[12px] 3xl:mt-[14px] mt-[16px] lg:mt-[9px] not-italic lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_800 text-left w-[51%]"
                        compId="1089:275"
                        comWidth={117}
                        comHeight={27}
                        compLeft={0}
                        compRight={10}
                        compType="Text"
                      >
                        <span className="text-gray_800 font-poppins">
                          <>{`Gaming `}</>
                        </span>
                        <span className="text-gray_500 font-poppins">
                          <>{`(40)`}</>
                        </span>
                      </Text>
                      <Text
                        className="font-normal mr-[10px] lg:mr-[5px] xl:mr-[6px] 2xl:mr-[7px] 3xl:mr-[9px] xl:mt-[10px] 2xl:mt-[12px] 3xl:mt-[14px] mt-[16px] lg:mt-[9px] not-italic lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_800 text-left w-[42%]"
                        compId="1089:276"
                        comWidth={96}
                        comHeight={27}
                        compLeft={0}
                        compRight={10}
                        compType="Text"
                      >
                        <span className="text-gray_800 font-poppins">
                          <>{`Printer `}</>
                        </span>
                        <span className="text-gray_500 font-poppins">
                          <>{`(14)`}</>
                        </span>
                      </Text>
                      <Text
                        className="font-normal mr-[10px] lg:mr-[5px] xl:mr-[6px] 2xl:mr-[7px] 3xl:mr-[9px] xl:mt-[10px] 2xl:mt-[12px] 3xl:mt-[14px] mt-[16px] lg:mt-[9px] not-italic lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_800 text-left w-[65%]"
                        compId="1089:277"
                        comWidth={150}
                        comHeight={27}
                        compLeft={0}
                        compRight={10}
                        compType="Text"
                      >
                        <span className="text-gray_800 font-poppins">
                          <>{`Accessories `}</>
                        </span>
                        <span className="text-gray_500 font-poppins">
                          <>{`(32)`}</>
                        </span>
                      </Text>
                      <Text
                        className="font-normal mr-[10px] lg:mr-[5px] xl:mr-[6px] 2xl:mr-[7px] 3xl:mr-[9px] xl:mt-[10px] 2xl:mt-[12px] 3xl:mt-[14px] mt-[16px] lg:mt-[9px] not-italic lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_800 text-left w-[81%]"
                        compId="1089:278"
                        comWidth={188}
                        comHeight={27}
                        compLeft={0}
                        compRight={10}
                        compType="Text"
                      >
                        <span className="text-gray_800 font-poppins">
                          <>{`More Categories `}</>
                        </span>
                        <span className="text-gray_500 font-poppins">
                          <>{`(10)`}</>
                        </span>
                      </Text>
                    </Column>
                  </Row>
                </Column>
              </Column>
              <Line
                className="bg-bluegray_100 h-[1px] lg:mt-[18px] xl:mt-[21px] 2xl:mt-[24px] 3xl:mt-[28px] mt-[32px] mx-[auto] w-[83%]"
                compId="1089:282"
                comWidth={312}
                comHeight={1}
                compLeft={32}
                compRight={32}
                compType="Line"
              />
              <Row
                className="items-center justify-between lg:mt-[18px] xl:mt-[20px] 2xl:mt-[23px] 3xl:mt-[27px] mt-[31px] mx-[auto] w-[83%]"
                compId="1089:288"
                comWidth={312}
                comHeight={27}
                compLeft={32}
                compRight={32}
                compType="Row"
              >
                <Text
                  className="font-medium lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_800 text-left w-[38%]"
                  compId="1089:289"
                  comWidth={120}
                  comHeight={27}
                  compLeft={0}
                  compRight={0}
                  compType="Text"
                >{`Filter by Price`}</Text>
                <Image
                  src={"images/img_chevrondown.svg"}
                  className="lg:h-[14px] xl:h-[17px] 2xl:h-[19px] 3xl:h-[22px] h-[24px] my-[1px] object-contain lg:w-[14px] xl:w-[16px] 2xl:w-[18px] 3xl:w-[21px] w-[24px]"
                  compId="1089:290"
                  comWidth={24}
                  comHeight={24}
                  compLeft={0}
                  compRight={0}
                  compType="Image"
                  alt="chevrondown"
                />
              </Row>
              <Stack
                className="lg:h-[14px] xl:h-[17px] 2xl:h-[19px] 3xl:h-[22px] h-[24px] lg:mt-[18px] xl:mt-[21px] 2xl:mt-[24px] 3xl:mt-[28px] mt-[32px] mx-[auto] w-[83%]"
                compId="1339"
                comWidth={312}
                comHeight={24}
                compLeft={32}
                compRight={32}
                compType="Stack"
              >
                <div
                  className="absolute bg-bluegray_100 lg:h-[4px] 2xl:h-[5px] xl:h-[5px] h-[6px] inset-x-[0] inset-y-[38%] w-[100%]"
                  compId="1089:285"
                  comWidth={312}
                  comHeight={6}
                  compLeft={0}
                  compRight={0}
                  compType="View"
                ></div>
                <Switch
                  compId="310"
                  comWidth={88}
                  comHeight={24}
                  compLeft={0}
                  compRight={0}
                  compType="Switch"
                  onColor="#f46b5b"
                  offColor=""
                  onHandleColor="#fafafa"
                  offHandleColor=""
                  checked={true}
                  className="absolute left-[0] w-[28%]"
                />
              </Stack>
              <Column
                className="items-start lg:mt-[14px] xl:mt-[16px] 2xl:mt-[18px] 3xl:mt-[21px] mt-[24px] w-[100%]"
                compId="1352"
                comWidth={376}
                comHeight={27}
                compLeft={0}
                compRight={0}
                compType="Column"
              >
                <Text
                  className="font-normal lg:mx-[18px] xl:mx-[21px] 2xl:mx-[24px] 3xl:mx-[28px] mx-[32px] not-italic lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_500 text-left w-[42%]"
                  compId="1089:292"
                  comWidth={159}
                  comHeight={27}
                  compLeft={32}
                  compRight={32}
                  compType="Text"
                >
                  <span className="text-gray_500 font-poppins">
                    <>{`Price: `}</>
                  </span>
                  <span className="text-gray_800 font-poppins font-medium">
                    <>{`$100 - $250`}</>
                  </span>
                  <span className="text-gray_500 font-poppins">
                    <>{` `}</>
                  </span>
                </Text>
              </Column>
              <Line
                className="bg-bluegray_100 h-[1px] lg:mt-[18px] xl:mt-[21px] 2xl:mt-[24px] 3xl:mt-[28px] mt-[32px] mx-[auto] w-[83%]"
                compId="1089:293"
                comWidth={312}
                comHeight={1}
                compLeft={32}
                compRight={32}
                compType="Line"
              />
              <Row
                className="items-center justify-between lg:mt-[18px] xl:mt-[20px] 2xl:mt-[23px] 3xl:mt-[27px] mt-[31px] mx-[auto] w-[83%]"
                compId="1089:317"
                comWidth={312}
                comHeight={27}
                compLeft={32}
                compRight={32}
                compType="Row"
              >
                <Text
                  className="font-medium lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_800 text-left w-[49%]"
                  compId="1089:318"
                  comWidth={153}
                  comHeight={27}
                  compLeft={0}
                  compRight={0}
                  compType="Text"
                >{`Filter by Location`}</Text>
                <Image
                  src={"images/img_chevrondown.svg"}
                  className="lg:h-[14px] xl:h-[17px] 2xl:h-[19px] 3xl:h-[22px] h-[24px] my-[1px] object-contain lg:w-[14px] xl:w-[16px] 2xl:w-[18px] 3xl:w-[21px] w-[24px]"
                  compId="1089:319"
                  comWidth={24}
                  comHeight={24}
                  compLeft={0}
                  compRight={0}
                  compType="Image"
                  alt="chevrondown"
                />
              </Row>
              <Column
                className="items-start lg:mt-[18px] xl:mt-[21px] 2xl:mt-[24px] 3xl:mt-[28px] mt-[32px] w-[100%]"
                compId="1353"
                comWidth={376}
                comHeight={231}
                compLeft={0}
                compRight={0}
                compType="Column"
              >
                <Column
                  className="items-start justify-start lg:mx-[18px] xl:mx-[21px] 2xl:mx-[24px] 3xl:mx-[28px] mx-[32px] w-[38%]"
                  compId="1089:295"
                  comWidth={144}
                  comHeight={231}
                  compLeft={32}
                  compRight={32}
                  compType="Column"
                >
                  <CheckBox
                    className="font-normal mr-[10px] lg:mr-[5px] xl:mr-[6px] 2xl:mr-[7px] 3xl:mr-[9px] not-italic lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_500 text-left w-[76%]"
                    inputClassName="2xl:h-[21px] 2xl:w-[20px] 3xl:h-[25px] 3xl:w-[24px] border-bluegray_100 border-bw3 border-solid h-[27px] lg:h-[16px] lg:w-[15px] w-[27px] xl:h-[19px] xl:w-[18px] mr-[5px]"
                    compId="306"
                    comWidth={109}
                    comHeight={27}
                    compLeft={0}
                    compRight={10}
                    compType="CheckBox"
                    name="Group306"
                    label={`Jakarta`}
                  ></CheckBox>
                  <Column
                    className="lg:mt-[14px] xl:mt-[16px] 2xl:mt-[18px] 3xl:mt-[21px] mt-[24px] w-[100%]"
                    compId="1356"
                    comWidth={144}
                    comHeight={27}
                    compLeft={0}
                    compRight={0}
                    compType="Column"
                  >
                    <Row
                      className="items-center justify-start w-[100%]"
                      compId="1089:300"
                      comWidth={144}
                      comHeight={27}
                      compLeft={0}
                      compRight={0}
                      compType="Row"
                    >
                      <Image
                        src={"images/img_checkedbox.svg"}
                        className="lg:h-[14px] xl:h-[17px] 2xl:h-[19px] 3xl:h-[22px] h-[24px] my-[1px] object-contain lg:w-[14px] xl:w-[16px] 2xl:w-[18px] 3xl:w-[21px] w-[24px]"
                        compId="1089:301"
                        comWidth={24}
                        comHeight={24}
                        compLeft={0}
                        compRight={0}
                        compType="Image"
                        alt="Checkedbox"
                      />
                      <Text
                        className="font-normal xl:ml-[10px] 2xl:ml-[12px] 3xl:ml-[14px] ml-[16px] lg:ml-[9px] not-italic text-deep_orange_400 lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-left w-[72%]"
                        compId="1089:304"
                        comWidth={104}
                        comHeight={27}
                        compLeft={16}
                        compRight={0}
                        compType="Text"
                      >{`Yogyakarta`}</Text>
                    </Row>
                  </Column>
                  <CheckBox
                    className="font-normal mr-[10px] lg:mr-[5px] xl:mr-[6px] 2xl:mr-[7px] 3xl:mr-[9px] lg:mt-[14px] xl:mt-[16px] 2xl:mt-[18px] 3xl:mt-[21px] mt-[24px] not-italic lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_500 text-left w-[85%]"
                    inputClassName="2xl:h-[21px] 2xl:w-[20px] 3xl:h-[25px] 3xl:w-[24px] border-bluegray_100 border-bw3 border-solid h-[27px] lg:h-[16px] lg:w-[15px] w-[27px] xl:h-[19px] xl:w-[18px] mr-[5px]"
                    compId="307"
                    comWidth={123}
                    comHeight={27}
                    compLeft={0}
                    compRight={10}
                    compType="CheckBox"
                    name="Group307"
                    label={`Bandung`}
                  ></CheckBox>
                  <CheckBox
                    className="font-normal lg:mr-[5px] 2xl:mr-[6px] xl:mr-[6px] 3xl:mr-[8px] mr-[9px] lg:mt-[14px] xl:mt-[16px] 2xl:mt-[18px] 3xl:mt-[21px] mt-[24px] not-italic lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_500 text-left w-[94%]"
                    inputClassName="2xl:h-[21px] 2xl:w-[20px] 3xl:h-[25px] 3xl:w-[24px] border-bluegray_100 border-bw3 border-solid h-[27px] lg:h-[16px] lg:w-[15px] w-[27px] xl:h-[19px] xl:w-[18px] mr-[5px]"
                    compId="308"
                    comWidth={135}
                    comHeight={27}
                    compLeft={0}
                    compRight={9}
                    compType="CheckBox"
                    name="Group308"
                    label={`Semarang`}
                  ></CheckBox>
                  <CheckBox
                    className="font-normal mr-[10px] lg:mr-[5px] xl:mr-[6px] 2xl:mr-[7px] 3xl:mr-[9px] lg:mt-[14px] xl:mt-[16px] 2xl:mt-[18px] 3xl:mt-[21px] mt-[24px] not-italic lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_500 text-left w-[89%]"
                    inputClassName="2xl:h-[21px] 2xl:w-[20px] 3xl:h-[25px] 3xl:w-[24px] border-bluegray_100 border-bw3 border-solid h-[27px] lg:h-[16px] lg:w-[15px] w-[27px] xl:h-[19px] xl:w-[18px] mr-[5px]"
                    compId="309"
                    comWidth={128}
                    comHeight={27}
                    compLeft={0}
                    compRight={10}
                    compType="CheckBox"
                    name="Group309"
                    label={`Surabaya`}
                  ></CheckBox>
                </Column>
              </Column>
              <Line
                className="bg-bluegray_100 h-[1px] lg:mt-[18px] xl:mt-[21px] 2xl:mt-[24px] 3xl:mt-[28px] mt-[32px] mx-[auto] w-[83%]"
                compId="1089:321"
                comWidth={312}
                comHeight={1}
                compLeft={32}
                compRight={32}
                compType="Line"
              />
              <Row
                className="items-center justify-between lg:mt-[18px] xl:mt-[20px] 2xl:mt-[23px] 3xl:mt-[27px] mt-[31px] mx-[auto] w-[83%]"
                compId="1089:360"
                comWidth={312}
                comHeight={27}
                compLeft={32}
                compRight={32}
                compType="Row"
              >
                <Text
                  className="font-medium lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_800 text-left w-[43%]"
                  compId="1089:361"
                  comWidth={135}
                  comHeight={27}
                  compLeft={0}
                  compRight={0}
                  compType="Text"
                >{`Filter by Rating`}</Text>
                <Image
                  src={"images/img_chevrondown.svg"}
                  className="lg:h-[14px] xl:h-[17px] 2xl:h-[19px] 3xl:h-[22px] h-[24px] my-[1px] object-contain lg:w-[14px] xl:w-[16px] 2xl:w-[18px] 3xl:w-[21px] w-[24px]"
                  compId="1089:362"
                  comWidth={24}
                  comHeight={24}
                  compLeft={0}
                  compRight={0}
                  compType="Image"
                  alt="chevrondown"
                />
              </Row>
              <Column
                className="items-start lg:mt-[18px] xl:mt-[21px] 2xl:mt-[24px] 3xl:mt-[28px] mt-[32px] w-[100%]"
                compId="1354"
                comWidth={376}
                comHeight={184}
                compLeft={0}
                compRight={0}
                compType="Column"
              >
                <Column
                  className="items-start justify-start lg:mx-[18px] xl:mx-[21px] 2xl:mx-[24px] 3xl:mx-[28px] mx-[32px] w-[51%]"
                  compId="1089:323"
                  comWidth={192}
                  comHeight={184}
                  compLeft={32}
                  compRight={32}
                  compType="Column"
                >
                  <Row
                    className="xl:gap-[10px] 2xl:gap-[12px] 3xl:gap-[14px] gap-[16px] lg:gap-[9px] grid grid-cols-2 items-center justify-start mr-[10px] lg:mr-[5px] xl:mr-[6px] 2xl:mr-[7px] 3xl:mr-[9px] w-[33%]"
                    compId="1089:324"
                    comWidth={64}
                    comHeight={24}
                    compLeft={0}
                    compRight={10}
                    compType="Row"
                  >
                    <Image
                      src={"images/img_checkbox.svg"}
                      className="lg:h-[14px] xl:h-[17px] 2xl:h-[19px] 3xl:h-[22px] h-[24px] object-contain lg:w-[14px] xl:w-[16px] 2xl:w-[18px] 3xl:w-[21px] w-[24px]"
                      compId="1089:327"
                      comWidth={24}
                      comHeight={24}
                      compLeft={0}
                      compRight={0}
                      compType="Image"
                      alt="Checkbox"
                    />
                    <Image
                      src={"images/img_star1_1.svg"}
                      className="lg:h-[14px] xl:h-[17px] 2xl:h-[19px] 3xl:h-[22px] h-[24px] object-contain lg:w-[14px] xl:w-[16px] 2xl:w-[18px] 3xl:w-[21px] w-[24px]"
                      compId="1089:326"
                      comWidth={24}
                      comHeight={24}
                      compLeft={0}
                      compRight={0}
                      compType="Image"
                      alt="Star1"
                    />
                  </Row>
                  <Row
                    className="items-center justify-start mr-[10px] lg:mr-[5px] xl:mr-[6px] 2xl:mr-[7px] 3xl:mr-[9px] xl:mt-[10px] 2xl:mt-[12px] 3xl:mt-[14px] mt-[16px] lg:mt-[9px] w-[50%]"
                    compId="1089:329"
                    comWidth={96}
                    comHeight={24}
                    compLeft={0}
                    compRight={10}
                    compType="Row"
                  >
                    <Image
                      src={"images/img_checkbox.svg"}
                      className="lg:h-[14px] xl:h-[17px] 2xl:h-[19px] 3xl:h-[22px] h-[24px] object-contain lg:w-[14px] xl:w-[16px] 2xl:w-[18px] 3xl:w-[21px] w-[24px]"
                      compId="1089:333"
                      comWidth={24}
                      comHeight={24}
                      compLeft={0}
                      compRight={0}
                      compType="Image"
                      alt="Checkbox"
                    />
                    <Row
                      className="lg:gap-[4px] xl:gap-[5px] 2xl:gap-[6px] 3xl:gap-[7px] gap-[8px] grid grid-cols-2 items-center justify-between xl:ml-[10px] 2xl:ml-[12px] 3xl:ml-[14px] ml-[16px] lg:ml-[9px] w-[58%]"
                      compId="1089:330"
                      comWidth={56}
                      comHeight={24}
                      compLeft={16}
                      compRight={0}
                      compType="Row"
                    >
                      <Image
                        src={"images/img_star1_1.svg"}
                        className="lg:h-[14px] xl:h-[17px] 2xl:h-[19px] 3xl:h-[22px] h-[24px] object-contain lg:w-[14px] xl:w-[16px] 2xl:w-[18px] 3xl:w-[21px] w-[24px]"
                        compId="1089:331"
                        comWidth={24}
                        comHeight={24}
                        compLeft={0}
                        compRight={0}
                        compType="Image"
                        alt="Star1"
                      />
                      <Image
                        src={"images/img_star1_1.svg"}
                        className="lg:h-[14px] xl:h-[17px] 2xl:h-[19px] 3xl:h-[22px] h-[24px] object-contain lg:w-[14px] xl:w-[16px] 2xl:w-[18px] 3xl:w-[21px] w-[24px]"
                        compId="1089:332"
                        comWidth={24}
                        comHeight={24}
                        compLeft={0}
                        compRight={0}
                        compType="Image"
                        alt="Star2"
                      />
                    </Row>
                  </Row>
                  <List
                    className="gap-[0] min-h-[auto] mr-[10px] lg:mr-[5px] xl:mr-[6px] 2xl:mr-[7px] 3xl:mr-[9px] xl:mt-[10px] 2xl:mt-[12px] 3xl:mt-[14px] mt-[16px] lg:mt-[9px] w-[83%]"
                    compId="1342"
                    comWidth={160}
                    comHeight={64}
                    compLeft={0}
                    compRight={10}
                    compType="List"
                    orientation="vertical"
                  >
                    <Row
                      className="items-center justify-between lg:mr-[18px] xl:mr-[21px] 2xl:mr-[24px] 3xl:mr-[28px] mr-[32px] lg:my-[4px] xl:my-[5px] 2xl:my-[6px] 3xl:my-[7px] my-[8px] w-[80%]"
                      compId="1089:335"
                      comWidth={128}
                      comHeight={24}
                      compLeft={0}
                      compRight={32}
                      compType="Row"
                    >
                      <Image
                        src={"images/img_checkbox.svg"}
                        className="lg:h-[14px] xl:h-[17px] 2xl:h-[19px] 3xl:h-[22px] h-[24px] object-contain lg:w-[14px] xl:w-[16px] 2xl:w-[18px] 3xl:w-[21px] w-[24px]"
                        compId="1089:340"
                        comWidth={24}
                        comHeight={24}
                        compLeft={0}
                        compRight={0}
                        compType="Image"
                        alt="Checkbox"
                      />
                      <Row
                        className="lg:gap-[4px] xl:gap-[5px] 2xl:gap-[6px] 3xl:gap-[7px] gap-[8px] grid grid-cols-3 items-center justify-between w-[69%]"
                        compId="1089:336"
                        comWidth={88}
                        comHeight={24}
                        compLeft={0}
                        compRight={0}
                        compType="Row"
                      >
                        <Image
                          src={"images/img_star1_1.svg"}
                          className="lg:h-[14px] xl:h-[17px] 2xl:h-[19px] 3xl:h-[22px] h-[24px] object-contain lg:w-[14px] xl:w-[16px] 2xl:w-[18px] 3xl:w-[21px] w-[24px]"
                          compId="1089:337"
                          comWidth={24}
                          comHeight={24}
                          compLeft={0}
                          compRight={0}
                          compType="Image"
                          alt="Star1"
                        />
                        <Image
                          src={"images/img_star1_1.svg"}
                          className="lg:h-[14px] xl:h-[17px] 2xl:h-[19px] 3xl:h-[22px] h-[24px] object-contain lg:w-[14px] xl:w-[16px] 2xl:w-[18px] 3xl:w-[21px] w-[24px]"
                          compId="1089:338"
                          comWidth={24}
                          comHeight={24}
                          compLeft={0}
                          compRight={0}
                          compType="Image"
                          alt="Star2"
                        />
                        <Image
                          src={"images/img_star1_1.svg"}
                          className="lg:h-[14px] xl:h-[17px] 2xl:h-[19px] 3xl:h-[22px] h-[24px] object-contain lg:w-[14px] xl:w-[16px] 2xl:w-[18px] 3xl:w-[21px] w-[24px]"
                          compId="1089:339"
                          comWidth={24}
                          comHeight={24}
                          compLeft={0}
                          compRight={0}
                          compType="Image"
                          alt="Star3"
                        />
                      </Row>
                    </Row>
                    <Row
                      className="items-center justify-between lg:my-[4px] xl:my-[5px] 2xl:my-[6px] 3xl:my-[7px] my-[8px] w-[100%]"
                      compId="1089:342"
                      comWidth={160}
                      comHeight={24}
                      compLeft={0}
                      compRight={0}
                      compType="Row"
                    >
                      <Image
                        src={"images/img_checkedbox.svg"}
                        className="lg:h-[14px] xl:h-[17px] 2xl:h-[19px] 3xl:h-[22px] h-[24px] object-contain lg:w-[14px] xl:w-[16px] 2xl:w-[18px] 3xl:w-[21px] w-[24px]"
                        compId="1089:348"
                        comWidth={24}
                        comHeight={24}
                        compLeft={0}
                        compRight={0}
                        compType="Image"
                        alt="Checkedbox"
                      />
                      <Row
                        className="lg:gap-[4px] xl:gap-[5px] 2xl:gap-[6px] 3xl:gap-[7px] gap-[8px] grid grid-cols-4 items-center justify-between w-[75%]"
                        compId="1089:343"
                        comWidth={120}
                        comHeight={24}
                        compLeft={0}
                        compRight={0}
                        compType="Row"
                      >
                        <Image
                          src={"images/img_star1_1.svg"}
                          className="lg:h-[14px] xl:h-[17px] 2xl:h-[19px] 3xl:h-[22px] h-[24px] object-contain lg:w-[14px] xl:w-[16px] 2xl:w-[18px] 3xl:w-[21px] w-[24px]"
                          compId="1089:344"
                          comWidth={24}
                          comHeight={24}
                          compLeft={0}
                          compRight={0}
                          compType="Image"
                          alt="Star1"
                        />
                        <Image
                          src={"images/img_star1_1.svg"}
                          className="lg:h-[14px] xl:h-[17px] 2xl:h-[19px] 3xl:h-[22px] h-[24px] object-contain lg:w-[14px] xl:w-[16px] 2xl:w-[18px] 3xl:w-[21px] w-[24px]"
                          compId="1089:345"
                          comWidth={24}
                          comHeight={24}
                          compLeft={0}
                          compRight={0}
                          compType="Image"
                          alt="Star2"
                        />
                        <Image
                          src={"images/img_star1_1.svg"}
                          className="lg:h-[14px] xl:h-[17px] 2xl:h-[19px] 3xl:h-[22px] h-[24px] object-contain lg:w-[14px] xl:w-[16px] 2xl:w-[18px] 3xl:w-[21px] w-[24px]"
                          compId="1089:346"
                          comWidth={24}
                          comHeight={24}
                          compLeft={0}
                          compRight={0}
                          compType="Image"
                          alt="Star3"
                        />
                        <Image
                          src={"images/img_star1_1.svg"}
                          className="lg:h-[14px] xl:h-[17px] 2xl:h-[19px] 3xl:h-[22px] h-[24px] object-contain lg:w-[14px] xl:w-[16px] 2xl:w-[18px] 3xl:w-[21px] w-[24px]"
                          compId="1089:347"
                          comWidth={24}
                          comHeight={24}
                          compLeft={0}
                          compRight={0}
                          compType="Image"
                          alt="Star5"
                        />
                      </Row>
                    </Row>
                  </List>
                  <Column
                    className="xl:mt-[10px] 2xl:mt-[12px] 3xl:mt-[14px] mt-[16px] lg:mt-[9px] w-[100%]"
                    compId="1357"
                    comWidth={192}
                    comHeight={24}
                    compLeft={0}
                    compRight={0}
                    compType="Column"
                  >
                    <Row
                      className="items-center justify-between w-[100%]"
                      compId="1089:351"
                      comWidth={192}
                      comHeight={24}
                      compLeft={0}
                      compRight={0}
                      compType="Row"
                    >
                      <Image
                        src={"images/img_checkbox.svg"}
                        className="lg:h-[14px] xl:h-[17px] 2xl:h-[19px] 3xl:h-[22px] h-[24px] object-contain lg:w-[14px] xl:w-[16px] 2xl:w-[18px] 3xl:w-[21px] w-[24px]"
                        compId="1089:358"
                        comWidth={24}
                        comHeight={24}
                        compLeft={0}
                        compRight={0}
                        compType="Image"
                        alt="Checkbox"
                      />
                      <RatingBar
                        className=""
                        compId="1344"
                        comWidth={152}
                        comHeight={24}
                        compLeft={0}
                        compRight={0}
                        compType="RatingBar"
                        value={5}
                        starCount={5}
                        activeColor="var(--orange_A100)"
                        size={((window.innerWidth - 15) * 24) / 1920}
                      ></RatingBar>
                    </Row>
                  </Column>
                </Column>
              </Column>
              <Line
                className="bg-bluegray_100 h-[1px] lg:mt-[18px] xl:mt-[21px] 2xl:mt-[24px] 3xl:mt-[28px] mt-[32px] mx-[auto] w-[83%]"
                compId="1089:364"
                comWidth={312}
                comHeight={1}
                compLeft={32}
                compRight={32}
                compType="Line"
              />
              <Row
                className="items-center justify-between lg:mt-[18px] xl:mt-[20px] 2xl:mt-[23px] 3xl:mt-[27px] mt-[31px] mx-[auto] w-[83%]"
                compId="1089:366"
                comWidth={312}
                comHeight={27}
                compLeft={32}
                compRight={32}
                compType="Row"
              >
                <Text
                  className="font-medium lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_800 text-left w-[42%]"
                  compId="1089:367"
                  comWidth={130}
                  comHeight={27}
                  compLeft={0}
                  compRight={0}
                  compType="Text"
                >{`Filter by Brand`}</Text>
                <Image
                  src={"images/img_chevrondown.svg"}
                  className="lg:h-[14px] xl:h-[17px] 2xl:h-[19px] 3xl:h-[22px] h-[24px] my-[1px] object-contain lg:w-[14px] xl:w-[16px] 2xl:w-[18px] 3xl:w-[21px] w-[24px]"
                  compId="1089:368"
                  comWidth={24}
                  comHeight={24}
                  compLeft={0}
                  compRight={0}
                  compType="Image"
                  alt="chevrondown"
                />
              </Row>
              <Row
                className="items-center justify-between lg:mt-[18px] xl:mt-[21px] 2xl:mt-[24px] 3xl:mt-[28px] mt-[32px] mx-[auto] w-[83%]"
                compId="1089:374"
                comWidth={312}
                comHeight={27}
                compLeft={32}
                compRight={32}
                compType="Row"
              >
                <Text
                  className="font-medium lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_800 text-left w-[54%]"
                  compId="1089:375"
                  comWidth={170}
                  comHeight={27}
                  compLeft={0}
                  compRight={0}
                  compType="Text"
                >{`Filter by Promotion`}</Text>
                <Image
                  src={"images/img_chevrondown.svg"}
                  className="lg:h-[14px] xl:h-[17px] 2xl:h-[19px] 3xl:h-[22px] h-[24px] my-[1px] object-contain lg:w-[14px] xl:w-[16px] 2xl:w-[18px] 3xl:w-[21px] w-[24px]"
                  compId="1089:376"
                  comWidth={24}
                  comHeight={24}
                  compLeft={0}
                  compRight={0}
                  compType="Image"
                  alt="chevrondown"
                />
              </Row>
              <Row
                className="items-center justify-between lg:mt-[18px] xl:mt-[20px] 2xl:mt-[23px] 3xl:mt-[27px] mt-[31px] mx-[auto] w-[83%]"
                compId="1089:370"
                comWidth={312}
                comHeight={27}
                compLeft={32}
                compRight={32}
                compType="Row"
              >
                <Text
                  className="font-medium lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_800 text-left w-[51%]"
                  compId="1089:371"
                  comWidth={158}
                  comHeight={27}
                  compLeft={0}
                  compRight={0}
                  compType="Text"
                >{`Filter by Payment`}</Text>
                <Image
                  src={"images/img_chevrondown.svg"}
                  className="lg:h-[14px] xl:h-[17px] 2xl:h-[19px] 3xl:h-[22px] h-[24px] my-[1px] object-contain lg:w-[14px] xl:w-[16px] 2xl:w-[18px] 3xl:w-[21px] w-[24px]"
                  compId="1089:372"
                  comWidth={24}
                  comHeight={24}
                  compLeft={0}
                  compRight={0}
                  compType="Image"
                  alt="chevrondown"
                />
              </Row>
              <Row
                className="items-center justify-between lg:mt-[18px] xl:mt-[21px] 2xl:mt-[24px] 3xl:mt-[28px] mt-[32px] mx-[auto] w-[83%]"
                compId="1089:378"
                comWidth={312}
                comHeight={27}
                compLeft={32}
                compRight={32}
                compType="Row"
              >
                <Text
                  className="font-medium lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_800 text-left w-[53%]"
                  compId="1089:379"
                  comWidth={164}
                  comHeight={27}
                  compLeft={0}
                  compRight={0}
                  compType="Text"
                >{`Filter by Shipment`}</Text>
                <Image
                  src={"images/img_chevrondown.svg"}
                  className="lg:h-[14px] xl:h-[17px] 2xl:h-[19px] 3xl:h-[22px] h-[24px] my-[1px] object-contain lg:w-[14px] xl:w-[16px] 2xl:w-[18px] 3xl:w-[21px] w-[24px]"
                  compId="1089:380"
                  comWidth={24}
                  comHeight={24}
                  compLeft={0}
                  compRight={0}
                  compType="Image"
                  alt="chevrondown"
                />
              </Row>
              <Button
                className="bg-deep_orange_400 font-medium lg:mt-[32px] xl:mt-[37px] 2xl:mt-[42px] 3xl:mt-[50px] mt-[56px] mx-[auto] lg:py-[10px] xl:py-[12px] 2xl:py-[13px] 3xl:py-[16px] py-[18.045px] text-center lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_50 w-[65%]"
                compId="291"
                comWidth={245}
                comHeight={60}
                compLeft={32}
                compRight={32}
                compType="Button"
              >{`Filter`}</Button>
              <Text
                className="font-medium lg:mt-[18px] xl:mt-[21px] 2xl:mt-[24px] 3xl:mt-[28px] mt-[32px] mx-[auto] text-deep_orange_400 lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-left w-[26%]"
                compId="1089:384"
                comWidth={99}
                comHeight={27}
                compLeft={32}
                compRight={32}
                compType="Text"
              >{`Reset Filter`}</Text>
            </Column>
            <Column
              className="items-center justify-start lg:mb-[42px] xl:mb-[48px] 2xl:mb-[54px] 3xl:mb-[65px] mb-[73px] lg:ml-[18px] xl:ml-[21px] 2xl:ml-[24px] 3xl:ml-[28px] ml-[32px] w-[62%]"
              compId="1331"
              comWidth={1192}
              comHeight={2008}
              compLeft={32}
              compRight={0}
              compType="Column"
            >
              <Column
                className="items-start justify-start w-[100%]"
                compId="1287"
                comWidth={1192}
                comHeight={120}
                compLeft={0}
                compRight={0}
                compType="Column"
              >
                <Text
                  className="font-normal lg:mx-[177px] xl:mx-[202px] 2xl:mx-[228px] 3xl:mx-[273px] mx-[304px] not-italic lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_800 text-left w-[15%]"
                  compId="889:963"
                  comWidth={176}
                  comHeight={27}
                  compLeft={304}
                  compRight={304}
                  compType="Text"
                >{`Your Lastest Search`}</Text>
                <Column
                  className="lg:mt-[19px] xl:mt-[22px] 2xl:mt-[25px] 3xl:mt-[30px] mt-[34px] w-[100%]"
                  compId="1358"
                  comWidth={1192}
                  comHeight={59}
                  compLeft={0}
                  compRight={0}
                  compType="Column"
                >
                  <Row
                    className="items-center justify-start w-[100%]"
                    compId="1286"
                    comWidth={1192}
                    comHeight={59}
                    compLeft={0}
                    compRight={0}
                    compType="Row"
                  >
                    <Button
                      className="bg-bluegray_100_7f font-normal lg:ml-[103px] xl:ml-[118px] 2xl:ml-[133px] 3xl:ml-[160px] ml-[178px] not-italic lg:py-[10px] xl:py-[11px] 2xl:py-[13px] 3xl:py-[15px] py-[17.545px] rounded-radius8 text-center lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_800 w-[12%]"
                      compId="294"
                      comWidth={145}
                      comHeight={59}
                      compLeft={178}
                      compRight={0}
                      compType="Button"
                    >{`Smartphone`}</Button>
                    <Button
                      className="bg-bluegray_100_7f font-normal xl:ml-[10px] 2xl:ml-[12px] 3xl:ml-[14px] ml-[16px] lg:ml-[9px] not-italic lg:py-[10px] xl:py-[11px] 2xl:py-[13px] 3xl:py-[15px] py-[17.545px] rounded-radius8 text-center lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_800 w-[9%]"
                      compId="293"
                      comWidth={104}
                      comHeight={59}
                      compLeft={16}
                      compRight={0}
                      compType="Button"
                    >{`Laptops`}</Button>
                    <Button
                      className="bg-bluegray_100_7f font-normal xl:ml-[10px] 2xl:ml-[12px] 3xl:ml-[14px] ml-[16px] lg:ml-[9px] not-italic lg:py-[10px] xl:py-[11px] 2xl:py-[13px] 3xl:py-[15px] py-[17.545px] rounded-radius8 text-center lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_800 w-[12%]"
                      compId="292"
                      comWidth={148}
                      comHeight={59}
                      compLeft={16}
                      compRight={0}
                      compType="Button"
                    >{`Headphones`}</Button>
                  </Row>
                </Column>
              </Column>
              <Row
                className="items-center justify-start lg:mt-[46px] xl:mt-[53px] 2xl:mt-[60px] 3xl:mt-[72px] mt-[80px] w-[100%]"
                compId="889:648"
                comWidth={1192}
                comHeight={27}
                compLeft={0}
                compRight={0}
                compType="Row"
              >
                <Text
                  className="font-medium lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_500 text-left w-[19%]"
                  compId="889:649"
                  comWidth={232}
                  comHeight={27}
                  compLeft={0}
                  compRight={0}
                  compType="Text"
                >
                  <span className="text-gray_500 font-poppins">
                    <>{`Viewing `}</>
                  </span>
                  <span className="text-gray_800 font-poppins">
                    <>{`20`}</>
                  </span>
                  <span className="text-gray_500 font-poppins">
                    <>{` of `}</>
                  </span>
                  <span className="text-gray_800 font-poppins">
                    <>{`160`}</>
                  </span>
                  <span className="text-gray_500 font-poppins">
                    <>{` product`}</>
                  </span>
                </Text>
                <Image
                  src={"images/img_mode.svg"}
                  className="lg:h-[14px] xl:h-[17px] 2xl:h-[19px] 3xl:h-[22px] h-[24px] lg:ml-[364px] xl:ml-[416px] 2xl:ml-[468px] 3xl:ml-[561px] ml-[624px] my-[1px] object-contain w-[6%]"
                  compId="1083:669"
                  comWidth={72}
                  comHeight={24}
                  compLeft={624}
                  compRight={0}
                  compType="Image"
                  alt="mode"
                />
                <Row
                  className="items-center justify-between lg:ml-[18px] xl:ml-[21px] 2xl:ml-[24px] 3xl:ml-[28px] ml-[32px] w-[19%]"
                  compId="889:650"
                  comWidth={232}
                  comHeight={27}
                  compLeft={32}
                  compRight={0}
                  compType="Row"
                >
                  <Text
                    className="font-normal not-italic lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_500 text-left w-[83%]"
                    compId="889:653"
                    comWidth={192}
                    comHeight={27}
                    compLeft={0}
                    compRight={0}
                    compType="Text"
                  >
                    <span className="text-gray_500 font-poppins">
                      <>{`Sort by: `}</>
                    </span>
                    <span className="text-gray_800 font-poppins">
                      <>{`Newest Items`}</>
                    </span>
                  </Text>
                  <Image
                    src={"images/img_chevrondown.svg"}
                    className="lg:h-[14px] xl:h-[17px] 2xl:h-[19px] 3xl:h-[22px] h-[24px] my-[1px] object-contain lg:w-[14px] xl:w-[16px] 2xl:w-[18px] 3xl:w-[21px] w-[24px]"
                    compId="889:651"
                    comWidth={24}
                    comHeight={24}
                    compLeft={0}
                    compRight={0}
                    compType="Image"
                    alt="chevrondown"
                  />
                </Row>
              </Row>
              <List
                className="gap-[0] min-h-[auto] lg:mt-[18px] xl:mt-[21px] 2xl:mt-[24px] 3xl:mt-[28px] mt-[32px] w-[100%]"
                compId="1343"
                comWidth={1192}
                comHeight={1618}
                compLeft={0}
                compRight={0}
                compType="List"
                orientation="vertical"
              >
                <Row
                  className="items-center justify-between xl:my-[10px] 2xl:my-[12px] 3xl:my-[14px] my-[16px] lg:my-[9px] w-[100%]"
                  compId="1294"
                  comWidth={1192}
                  comHeight={243}
                  compLeft={0}
                  compRight={0}
                  compType="Row"
                >
                  <Image
                    src={"images/img_placeholder_8.svg"}
                    className="lg:h-[142px] xl:h-[163px] 2xl:h-[183px] 3xl:h-[219px] h-[243px] object-contain w-[32%]"
                    compId="1083:361"
                    comWidth={376}
                    comHeight={243}
                    compLeft={0}
                    compRight={0}
                    compType="Image"
                    alt="placeholder"
                  />
                  <Column
                    className="mb-[1px] w-[66%]"
                    compId="1293"
                    comWidth={784}
                    comHeight={242}
                    compLeft={0}
                    compRight={0}
                    compType="Column"
                  >
                    <Row
                      className="items-start justify-between w-[100%]"
                      compId="1292"
                      comWidth={784}
                      comHeight={167}
                      compLeft={0}
                      compRight={0}
                      compType="Row"
                    >
                      <Column
                        className="items-start justify-start mb-[10px] lg:mb-[5px] xl:mb-[6px] 2xl:mb-[7px] 3xl:mb-[9px] w-[65%]"
                        compId="1083:350"
                        comWidth={512}
                        comHeight={157}
                        compLeft={0}
                        compRight={0}
                        compType="Column"
                      >
                        <Row
                          className="items-center justify-start mr-[10px] lg:mr-[5px] xl:mr-[6px] 2xl:mr-[7px] 3xl:mr-[9px] w-[76%]"
                          compId="1083:695"
                          comWidth={387}
                          comHeight={27}
                          compLeft={0}
                          compRight={10}
                          compType="Row"
                        >
                          <Row
                            className="items-center justify-between w-[44%]"
                            compId="1083:696"
                            comWidth={172}
                            comHeight={27}
                            compLeft={0}
                            compRight={0}
                            compType="Row"
                          >
                            <Text
                              className="font-medium lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_800 text-left w-[16%]"
                              compId="1083:697"
                              comWidth={28}
                              comHeight={27}
                              compLeft={0}
                              compRight={0}
                              compType="Text"
                            >{`5.0`}</Text>
                            <RatingBar
                              className="my-[1px]"
                              compId="1345"
                              comWidth={136}
                              comHeight={24}
                              compLeft={0}
                              compRight={0}
                              compType="RatingBar"
                              value={5}
                              starCount={5}
                              activeColor="var(--orange_A100)"
                              size={((window.innerWidth - 15) * 24) / 1920}
                            ></RatingBar>
                          </Row>
                          <Text
                            className="font-normal xl:ml-[10px] 2xl:ml-[12px] 3xl:ml-[14px] ml-[16px] lg:ml-[9px] not-italic lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_500 text-left w-[26%]"
                            compId="1083:704"
                            comWidth={100}
                            comHeight={27}
                            compLeft={16}
                            compRight={0}
                            compType="Text"
                          >{`Review (12)`}</Text>
                          <Line
                            className="bg-gray_500 lg:h-[13px] xl:h-[15px] 2xl:h-[17px] 3xl:h-[20px] h-[22px] xl:ml-[10px] 2xl:ml-[12px] 3xl:ml-[14px] ml-[16px] lg:ml-[9px] 2xl:my-[1px] xl:my-[1px] lg:my-[1px] my-[2.5px] 3xl:my-[2px] opacity-op5 rotate-[90deg] w-[2px]"
                            compId="1083:705"
                            comWidth={2}
                            comHeight={22}
                            compLeft={16}
                            compRight={0}
                            compType="Line"
                          />
                          <Text
                            className="font-normal 2xl:ml-[10px] 3xl:ml-[12px] ml-[14px] lg:ml-[8px] xl:ml-[9px] not-italic opacity-op5 lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_800 text-left w-[17%]"
                            compId="1083:706"
                            comWidth={67}
                            comHeight={27}
                            compLeft={14}
                            compRight={0}
                            compType="Text"
                          >{`Sold 99`}</Text>
                        </Row>
                        <Text
                          className="font-medium mr-[10px] lg:mr-[5px] xl:mr-[6px] 2xl:mr-[7px] 3xl:mr-[9px] lg:mt-[14px] xl:mt-[16px] 2xl:mt-[18px] 3xl:mt-[21px] mt-[24px] lg:text-fs14 xl:text-fs16 2xl:text-fs18 3xl:text-fs21 text-fs24 text-gray_800 text-left w-[51%]"
                          compId="1083:351"
                          comWidth={260}
                          comHeight={36}
                          compLeft={0}
                          compRight={10}
                          compType="Text"
                        >{`USB Speaker Portable`}</Text>
                        <Text
                          className="font-normal leading-lh xl:mt-[10px] 2xl:mt-[12px] 3xl:mt-[14px] mt-[16px] lg:mt-[9px] not-italic opacity-op5 lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_800 text-left w-[100%]"
                          compId="1083:352"
                          comWidth={512}
                          comHeight={54}
                          compLeft={0}
                          compRight={0}
                          compType="Text"
                        >{`Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore.`}</Text>
                      </Column>
                      <Column
                        className="items-start justify-start lg:mt-[44px] xl:mt-[51px] 2xl:mt-[57px] 3xl:mt-[69px] mt-[77px] w-[16%]"
                        compId="1291"
                        comWidth={122}
                        comHeight={90}
                        compLeft={0}
                        compRight={0}
                        compType="Column"
                      >
                        <Text
                          className="font-normal line-through ml-[10px] lg:ml-[5px] xl:ml-[6px] 2xl:ml-[7px] 3xl:ml-[9px] not-italic lg:text-fs14 xl:text-fs16 2xl:text-fs18 3xl:text-fs21 text-fs24 text-gray_500 text-left w-[66%]"
                          compId="1083:385"
                          comWidth={80}
                          comHeight={36}
                          compLeft={10}
                          compRight={0}
                          compType="Text"
                        >{`$39.99`}</Text>
                        <Column
                          className="w-[100%]"
                          compId="1359"
                          comWidth={122}
                          comHeight={54}
                          compLeft={0}
                          compRight={0}
                          compType="Column"
                        >
                          <Text
                            className="font-medium text-deep_orange_400 lg:text-fs21 xl:text-fs24 2xl:text-fs27 3xl:text-fs32 text-fs36 text-left w-[100%]"
                            compId="1083:384"
                            comWidth={122}
                            comHeight={54}
                            compLeft={0}
                            compRight={0}
                            compType="Text"
                          >{`$29.99`}</Text>
                        </Column>
                      </Column>
                    </Row>
                    <Row
                      className="items-center justify-start xl:mt-[10px] 2xl:mt-[11px] 3xl:mt-[13px] mt-[15px] lg:mt-[8px] w-[100%]"
                      compId="1083:386"
                      comWidth={784}
                      comHeight={60}
                      compLeft={0}
                      compRight={0}
                      compType="Row"
                    >
                      <Button
                        className="bg-deep_orange_400 font-medium lg:py-[10px] xl:py-[12px] 2xl:py-[13px] 3xl:py-[16px] py-[18.045px] rounded-radius8 text-center lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_50 w-[23%]"
                        compId="295"
                        comWidth={180}
                        comHeight={60}
                        compLeft={0}
                        compRight={0}
                        compType="Button"
                      >{`Add to Cart`}</Button>
                      <Image
                        src={"images/img_love_1.svg"}
                        className="lg:h-[14px] xl:h-[17px] 2xl:h-[19px] 3xl:h-[22px] h-[24px] xl:mb-[11px] 3xl:mb-[15px] mb-[17.5px] lg:ml-[254px] xl:ml-[291px] 2xl:ml-[327px] 3xl:ml-[393px] ml-[437px] xl:mt-[12px] 3xl:mt-[16px] mt-[18.5px] lg:my-[10px] 2xl:my-[13px] object-contain lg:w-[14px] xl:w-[16px] 2xl:w-[18px] 3xl:w-[21px] w-[24px]"
                        compId="1083:387"
                        comWidth={24}
                        comHeight={24}
                        compLeft={437}
                        compRight={0}
                        compType="Image"
                        alt="Love"
                      />
                      <Text
                        className="font-medium xl:mb-[10px] 3xl:mb-[14px] mb-[16px] lg:ml-[4px] xl:ml-[5px] 2xl:ml-[6px] 3xl:ml-[7px] ml-[8px] xl:mt-[11px] 3xl:mt-[15px] mt-[17px] 2xl:my-[12px] lg:my-[9px] lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_800 text-left w-[17%]"
                        compId="1083:389"
                        comWidth={135}
                        comHeight={27}
                        compLeft={8}
                        compRight={0}
                        compType="Text"
                      >{`Add to Wishlist`}</Text>
                    </Row>
                  </Column>
                </Row>
                <Row
                  className="items-center justify-between xl:my-[10px] 2xl:my-[12px] 3xl:my-[14px] my-[16px] lg:my-[9px] w-[100%]"
                  compId="1298"
                  comWidth={1192}
                  comHeight={243}
                  compLeft={0}
                  compRight={0}
                  compType="Row"
                >
                  <Image
                    src={"images/img_placeholder_9.svg"}
                    className="lg:h-[142px] xl:h-[163px] 2xl:h-[183px] 3xl:h-[219px] h-[243px] object-contain w-[32%]"
                    compId="1083:712"
                    comWidth={376}
                    comHeight={243}
                    compLeft={0}
                    compRight={0}
                    compType="Image"
                    alt="placeholder"
                  />
                  <Column
                    className="mb-[1px] w-[66%]"
                    compId="1297"
                    comWidth={784}
                    comHeight={242}
                    compLeft={0}
                    compRight={0}
                    compType="Column"
                  >
                    <Row
                      className="items-start justify-between w-[100%]"
                      compId="1296"
                      comWidth={784}
                      comHeight={167}
                      compLeft={0}
                      compRight={0}
                      compType="Row"
                    >
                      <Column
                        className="items-start justify-start mb-[10px] lg:mb-[5px] xl:mb-[6px] 2xl:mb-[7px] 3xl:mb-[9px] w-[65%]"
                        compId="1083:709"
                        comWidth={512}
                        comHeight={157}
                        compLeft={0}
                        compRight={0}
                        compType="Column"
                      >
                        <Row
                          className="items-center justify-start mr-[10px] lg:mr-[5px] xl:mr-[6px] 2xl:mr-[7px] 3xl:mr-[9px] w-[76%]"
                          compId="1083:724"
                          comWidth={387}
                          comHeight={27}
                          compLeft={0}
                          compRight={10}
                          compType="Row"
                        >
                          <Row
                            className="items-center justify-between w-[44%]"
                            compId="1083:725"
                            comWidth={172}
                            comHeight={27}
                            compLeft={0}
                            compRight={0}
                            compType="Row"
                          >
                            <Text
                              className="font-medium lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_800 text-left w-[16%]"
                              compId="1083:726"
                              comWidth={28}
                              comHeight={27}
                              compLeft={0}
                              compRight={0}
                              compType="Text"
                            >{`5.0`}</Text>
                            <RatingBar
                              className="my-[1px]"
                              compId="1346"
                              comWidth={136}
                              comHeight={24}
                              compLeft={0}
                              compRight={0}
                              compType="RatingBar"
                              value={5}
                              starCount={5}
                              activeColor="var(--orange_A100)"
                              size={((window.innerWidth - 15) * 24) / 1920}
                            ></RatingBar>
                          </Row>
                          <Text
                            className="font-normal xl:ml-[10px] 2xl:ml-[12px] 3xl:ml-[14px] ml-[16px] lg:ml-[9px] not-italic lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_500 text-left w-[26%]"
                            compId="1083:733"
                            comWidth={100}
                            comHeight={27}
                            compLeft={16}
                            compRight={0}
                            compType="Text"
                          >{`Review (12)`}</Text>
                          <Line
                            className="bg-gray_500 lg:h-[13px] xl:h-[15px] 2xl:h-[17px] 3xl:h-[20px] h-[22px] xl:ml-[10px] 2xl:ml-[12px] 3xl:ml-[14px] ml-[16px] lg:ml-[9px] 2xl:my-[1px] xl:my-[1px] lg:my-[1px] my-[2.5px] 3xl:my-[2px] opacity-op5 rotate-[90deg] w-[2px]"
                            compId="1083:734"
                            comWidth={2}
                            comHeight={22}
                            compLeft={16}
                            compRight={0}
                            compType="Line"
                          />
                          <Text
                            className="font-normal 2xl:ml-[10px] 3xl:ml-[12px] ml-[14px] lg:ml-[8px] xl:ml-[9px] not-italic opacity-op5 lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_800 text-left w-[17%]"
                            compId="1083:735"
                            comWidth={67}
                            comHeight={27}
                            compLeft={14}
                            compRight={0}
                            compType="Text"
                          >{`Sold 99`}</Text>
                        </Row>
                        <Text
                          className="font-medium mr-[10px] lg:mr-[5px] xl:mr-[6px] 2xl:mr-[7px] 3xl:mr-[9px] lg:mt-[14px] xl:mt-[16px] 2xl:mt-[18px] 3xl:mt-[21px] mt-[24px] lg:text-fs14 xl:text-fs16 2xl:text-fs18 3xl:text-fs21 text-fs24 text-gray_800 text-left w-[28%]"
                          compId="1083:710"
                          comWidth={144}
                          comHeight={36}
                          compLeft={0}
                          compRight={10}
                          compType="Text"
                        >{`Joystick Pro`}</Text>
                        <Text
                          className="font-normal leading-lh xl:mt-[10px] 2xl:mt-[12px] 3xl:mt-[14px] mt-[16px] lg:mt-[9px] not-italic opacity-op5 lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_800 text-left w-[100%]"
                          compId="1083:711"
                          comWidth={512}
                          comHeight={54}
                          compLeft={0}
                          compRight={0}
                          compType="Text"
                        >{`Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore.`}</Text>
                      </Column>
                      <Column
                        className="items-start justify-start lg:mt-[44px] xl:mt-[51px] 2xl:mt-[57px] 3xl:mt-[69px] mt-[77px] w-[16%]"
                        compId="1295"
                        comWidth={122}
                        comHeight={90}
                        compLeft={0}
                        compRight={0}
                        compType="Column"
                      >
                        <Text
                          className="font-normal line-through ml-[10px] lg:ml-[5px] xl:ml-[6px] 2xl:ml-[7px] 3xl:ml-[9px] not-italic lg:text-fs14 xl:text-fs16 2xl:text-fs18 3xl:text-fs21 text-fs24 text-gray_500 text-left w-[66%]"
                          compId="1083:717"
                          comWidth={80}
                          comHeight={36}
                          compLeft={10}
                          compRight={0}
                          compType="Text"
                        >{`$39.99`}</Text>
                        <Column
                          className="w-[100%]"
                          compId="1360"
                          comWidth={122}
                          comHeight={54}
                          compLeft={0}
                          compRight={0}
                          compType="Column"
                        >
                          <Text
                            className="font-medium text-deep_orange_400 lg:text-fs21 xl:text-fs24 2xl:text-fs27 3xl:text-fs32 text-fs36 text-left w-[100%]"
                            compId="1083:716"
                            comWidth={122}
                            comHeight={54}
                            compLeft={0}
                            compRight={0}
                            compType="Text"
                          >{`$29.99`}</Text>
                        </Column>
                      </Column>
                    </Row>
                    <Row
                      className="items-center justify-start xl:mt-[10px] 2xl:mt-[11px] 3xl:mt-[13px] mt-[15px] lg:mt-[8px] w-[100%]"
                      compId="1083:718"
                      comWidth={784}
                      comHeight={60}
                      compLeft={0}
                      compRight={0}
                      compType="Row"
                    >
                      <Button
                        className="bg-deep_orange_400 font-medium lg:py-[10px] xl:py-[12px] 2xl:py-[13px] 3xl:py-[16px] py-[18.045px] rounded-radius8 text-center lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_50 w-[23%]"
                        compId="297"
                        comWidth={180}
                        comHeight={60}
                        compLeft={0}
                        compRight={0}
                        compType="Button"
                      >{`Add to Cart`}</Button>
                      <Image
                        src={"images/img_love_1.svg"}
                        className="lg:h-[14px] xl:h-[17px] 2xl:h-[19px] 3xl:h-[22px] h-[24px] xl:mb-[11px] 3xl:mb-[15px] mb-[17.5px] lg:ml-[254px] xl:ml-[291px] 2xl:ml-[327px] 3xl:ml-[393px] ml-[437px] xl:mt-[12px] 3xl:mt-[16px] mt-[18.5px] lg:my-[10px] 2xl:my-[13px] object-contain lg:w-[14px] xl:w-[16px] 2xl:w-[18px] 3xl:w-[21px] w-[24px]"
                        compId="1083:719"
                        comWidth={24}
                        comHeight={24}
                        compLeft={437}
                        compRight={0}
                        compType="Image"
                        alt="Love"
                      />
                      <Text
                        className="font-medium xl:mb-[10px] 3xl:mb-[14px] mb-[16px] lg:ml-[4px] xl:ml-[5px] 2xl:ml-[6px] 3xl:ml-[7px] ml-[8px] xl:mt-[11px] 3xl:mt-[15px] mt-[17px] 2xl:my-[12px] lg:my-[9px] lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_800 text-left w-[17%]"
                        compId="1083:721"
                        comWidth={135}
                        comHeight={27}
                        compLeft={8}
                        compRight={0}
                        compType="Text"
                      >{`Add to Wishlist`}</Text>
                    </Row>
                  </Column>
                </Row>
                <Row
                  className="items-center justify-between xl:my-[10px] 2xl:my-[12px] 3xl:my-[14px] my-[16px] lg:my-[9px] w-[100%]"
                  compId="1302"
                  comWidth={1192}
                  comHeight={243}
                  compLeft={0}
                  compRight={0}
                  compType="Row"
                >
                  <Image
                    src={"images/img_placeholder_10.svg"}
                    className="lg:h-[142px] xl:h-[163px] 2xl:h-[183px] 3xl:h-[219px] h-[243px] object-contain w-[32%]"
                    compId="1083:740"
                    comWidth={376}
                    comHeight={243}
                    compLeft={0}
                    compRight={0}
                    compType="Image"
                    alt="placeholder"
                  />
                  <Column
                    className="mb-[1px] w-[66%]"
                    compId="1301"
                    comWidth={784}
                    comHeight={242}
                    compLeft={0}
                    compRight={0}
                    compType="Column"
                  >
                    <Row
                      className="items-start justify-between w-[100%]"
                      compId="1300"
                      comWidth={784}
                      comHeight={167}
                      compLeft={0}
                      compRight={0}
                      compType="Row"
                    >
                      <Column
                        className="items-start justify-start mb-[10px] lg:mb-[5px] xl:mb-[6px] 2xl:mb-[7px] 3xl:mb-[9px] w-[65%]"
                        compId="1083:737"
                        comWidth={512}
                        comHeight={157}
                        compLeft={0}
                        compRight={0}
                        compType="Column"
                      >
                        <Row
                          className="items-center justify-start mr-[10px] lg:mr-[5px] xl:mr-[6px] 2xl:mr-[7px] 3xl:mr-[9px] w-[76%]"
                          compId="1083:752"
                          comWidth={387}
                          comHeight={27}
                          compLeft={0}
                          compRight={10}
                          compType="Row"
                        >
                          <Row
                            className="items-center justify-between w-[44%]"
                            compId="1083:753"
                            comWidth={172}
                            comHeight={27}
                            compLeft={0}
                            compRight={0}
                            compType="Row"
                          >
                            <Text
                              className="font-medium lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_800 text-left w-[16%]"
                              compId="1083:754"
                              comWidth={28}
                              comHeight={27}
                              compLeft={0}
                              compRight={0}
                              compType="Text"
                            >{`5.0`}</Text>
                            <RatingBar
                              className="my-[1px]"
                              compId="1347"
                              comWidth={136}
                              comHeight={24}
                              compLeft={0}
                              compRight={0}
                              compType="RatingBar"
                              value={5}
                              starCount={5}
                              activeColor="var(--orange_A100)"
                              size={((window.innerWidth - 15) * 24) / 1920}
                            ></RatingBar>
                          </Row>
                          <Text
                            className="font-normal xl:ml-[10px] 2xl:ml-[12px] 3xl:ml-[14px] ml-[16px] lg:ml-[9px] not-italic lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_500 text-left w-[26%]"
                            compId="1083:761"
                            comWidth={100}
                            comHeight={27}
                            compLeft={16}
                            compRight={0}
                            compType="Text"
                          >{`Review (12)`}</Text>
                          <Line
                            className="bg-gray_500 lg:h-[13px] xl:h-[15px] 2xl:h-[17px] 3xl:h-[20px] h-[22px] xl:ml-[10px] 2xl:ml-[12px] 3xl:ml-[14px] ml-[16px] lg:ml-[9px] 2xl:my-[1px] xl:my-[1px] lg:my-[1px] my-[2.5px] 3xl:my-[2px] opacity-op5 rotate-[90deg] w-[2px]"
                            compId="1083:762"
                            comWidth={2}
                            comHeight={22}
                            compLeft={16}
                            compRight={0}
                            compType="Line"
                          />
                          <Text
                            className="font-normal 2xl:ml-[10px] 3xl:ml-[12px] ml-[14px] lg:ml-[8px] xl:ml-[9px] not-italic opacity-op5 lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_800 text-left w-[17%]"
                            compId="1083:763"
                            comWidth={67}
                            comHeight={27}
                            compLeft={14}
                            compRight={0}
                            compType="Text"
                          >{`Sold 99`}</Text>
                        </Row>
                        <Text
                          className="font-medium mr-[10px] lg:mr-[5px] xl:mr-[6px] 2xl:mr-[7px] 3xl:mr-[9px] lg:mt-[14px] xl:mt-[16px] 2xl:mt-[18px] 3xl:mt-[21px] mt-[24px] lg:text-fs14 xl:text-fs16 2xl:text-fs18 3xl:text-fs21 text-fs24 text-gray_800 text-left w-[38%]"
                          compId="1083:738"
                          comWidth={192}
                          comHeight={36}
                          compLeft={0}
                          compRight={10}
                          compType="Text"
                        >{`Smartphone 5G`}</Text>
                        <Text
                          className="font-normal leading-lh xl:mt-[10px] 2xl:mt-[12px] 3xl:mt-[14px] mt-[16px] lg:mt-[9px] not-italic opacity-op5 lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_800 text-left w-[100%]"
                          compId="1083:739"
                          comWidth={512}
                          comHeight={54}
                          compLeft={0}
                          compRight={0}
                          compType="Text"
                        >{`Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore.`}</Text>
                      </Column>
                      <Column
                        className="items-start justify-start lg:mt-[44px] xl:mt-[51px] 2xl:mt-[57px] 3xl:mt-[69px] mt-[77px] w-[16%]"
                        compId="1299"
                        comWidth={122}
                        comHeight={90}
                        compLeft={0}
                        compRight={0}
                        compType="Column"
                      >
                        <Text
                          className="font-normal line-through ml-[10px] lg:ml-[5px] xl:ml-[6px] 2xl:ml-[7px] 3xl:ml-[9px] not-italic lg:text-fs14 xl:text-fs16 2xl:text-fs18 3xl:text-fs21 text-fs24 text-gray_500 text-left w-[66%]"
                          compId="1083:745"
                          comWidth={80}
                          comHeight={36}
                          compLeft={10}
                          compRight={0}
                          compType="Text"
                        >{`$39.99`}</Text>
                        <Column
                          className="w-[100%]"
                          compId="1361"
                          comWidth={122}
                          comHeight={54}
                          compLeft={0}
                          compRight={0}
                          compType="Column"
                        >
                          <Text
                            className="font-medium text-deep_orange_400 lg:text-fs21 xl:text-fs24 2xl:text-fs27 3xl:text-fs32 text-fs36 text-left w-[100%]"
                            compId="1083:744"
                            comWidth={122}
                            comHeight={54}
                            compLeft={0}
                            compRight={0}
                            compType="Text"
                          >{`$29.99`}</Text>
                        </Column>
                      </Column>
                    </Row>
                    <Row
                      className="items-center justify-start xl:mt-[10px] 2xl:mt-[11px] 3xl:mt-[13px] mt-[15px] lg:mt-[8px] w-[100%]"
                      compId="1083:746"
                      comWidth={784}
                      comHeight={60}
                      compLeft={0}
                      compRight={0}
                      compType="Row"
                    >
                      <Button
                        className="bg-deep_orange_400 font-medium lg:py-[10px] xl:py-[12px] 2xl:py-[13px] 3xl:py-[16px] py-[18.045px] rounded-radius8 text-center lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_50 w-[23%]"
                        compId="299"
                        comWidth={180}
                        comHeight={60}
                        compLeft={0}
                        compRight={0}
                        compType="Button"
                      >{`Add to Cart`}</Button>
                      <Image
                        src={"images/img_love_1.svg"}
                        className="lg:h-[14px] xl:h-[17px] 2xl:h-[19px] 3xl:h-[22px] h-[24px] xl:mb-[11px] 3xl:mb-[15px] mb-[17.5px] lg:ml-[254px] xl:ml-[291px] 2xl:ml-[327px] 3xl:ml-[393px] ml-[437px] xl:mt-[12px] 3xl:mt-[16px] mt-[18.5px] lg:my-[10px] 2xl:my-[13px] object-contain lg:w-[14px] xl:w-[16px] 2xl:w-[18px] 3xl:w-[21px] w-[24px]"
                        compId="1083:747"
                        comWidth={24}
                        comHeight={24}
                        compLeft={437}
                        compRight={0}
                        compType="Image"
                        alt="Love"
                      />
                      <Text
                        className="font-medium xl:mb-[10px] 3xl:mb-[14px] mb-[16px] lg:ml-[4px] xl:ml-[5px] 2xl:ml-[6px] 3xl:ml-[7px] ml-[8px] xl:mt-[11px] 3xl:mt-[15px] mt-[17px] 2xl:my-[12px] lg:my-[9px] lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_800 text-left w-[17%]"
                        compId="1083:749"
                        comWidth={135}
                        comHeight={27}
                        compLeft={8}
                        compRight={0}
                        compType="Text"
                      >{`Add to Wishlist`}</Text>
                    </Row>
                  </Column>
                </Row>
                <Row
                  className="items-center justify-between xl:my-[10px] 2xl:my-[12px] 3xl:my-[14px] my-[16px] lg:my-[9px] w-[100%]"
                  compId="1306"
                  comWidth={1192}
                  comHeight={243}
                  compLeft={0}
                  compRight={0}
                  compType="Row"
                >
                  <Image
                    src={"images/img_placeholder_11.svg"}
                    className="lg:h-[142px] xl:h-[163px] 2xl:h-[183px] 3xl:h-[219px] h-[243px] object-contain w-[32%]"
                    compId="1083:768"
                    comWidth={376}
                    comHeight={243}
                    compLeft={0}
                    compRight={0}
                    compType="Image"
                    alt="placeholder"
                  />
                  <Column
                    className="mb-[1px] w-[66%]"
                    compId="1305"
                    comWidth={784}
                    comHeight={242}
                    compLeft={0}
                    compRight={0}
                    compType="Column"
                  >
                    <Row
                      className="items-start justify-between w-[100%]"
                      compId="1304"
                      comWidth={784}
                      comHeight={167}
                      compLeft={0}
                      compRight={0}
                      compType="Row"
                    >
                      <Column
                        className="items-start justify-start mb-[10px] lg:mb-[5px] xl:mb-[6px] 2xl:mb-[7px] 3xl:mb-[9px] w-[65%]"
                        compId="1083:765"
                        comWidth={512}
                        comHeight={157}
                        compLeft={0}
                        compRight={0}
                        compType="Column"
                      >
                        <Row
                          className="items-center justify-start mr-[10px] lg:mr-[5px] xl:mr-[6px] 2xl:mr-[7px] 3xl:mr-[9px] w-[76%]"
                          compId="1083:780"
                          comWidth={387}
                          comHeight={27}
                          compLeft={0}
                          compRight={10}
                          compType="Row"
                        >
                          <Row
                            className="items-center justify-between w-[44%]"
                            compId="1083:781"
                            comWidth={172}
                            comHeight={27}
                            compLeft={0}
                            compRight={0}
                            compType="Row"
                          >
                            <Text
                              className="font-medium lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_800 text-left w-[16%]"
                              compId="1083:782"
                              comWidth={28}
                              comHeight={27}
                              compLeft={0}
                              compRight={0}
                              compType="Text"
                            >{`5.0`}</Text>
                            <RatingBar
                              className="my-[1px]"
                              compId="1348"
                              comWidth={136}
                              comHeight={24}
                              compLeft={0}
                              compRight={0}
                              compType="RatingBar"
                              value={5}
                              starCount={5}
                              activeColor="var(--orange_A100)"
                              size={((window.innerWidth - 15) * 24) / 1920}
                            ></RatingBar>
                          </Row>
                          <Text
                            className="font-normal xl:ml-[10px] 2xl:ml-[12px] 3xl:ml-[14px] ml-[16px] lg:ml-[9px] not-italic lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_500 text-left w-[26%]"
                            compId="1083:789"
                            comWidth={100}
                            comHeight={27}
                            compLeft={16}
                            compRight={0}
                            compType="Text"
                          >{`Review (12)`}</Text>
                          <Line
                            className="bg-gray_500 lg:h-[13px] xl:h-[15px] 2xl:h-[17px] 3xl:h-[20px] h-[22px] xl:ml-[10px] 2xl:ml-[12px] 3xl:ml-[14px] ml-[16px] lg:ml-[9px] 2xl:my-[1px] xl:my-[1px] lg:my-[1px] my-[2.5px] 3xl:my-[2px] opacity-op5 rotate-[90deg] w-[2px]"
                            compId="1083:790"
                            comWidth={2}
                            comHeight={22}
                            compLeft={16}
                            compRight={0}
                            compType="Line"
                          />
                          <Text
                            className="font-normal 2xl:ml-[10px] 3xl:ml-[12px] ml-[14px] lg:ml-[8px] xl:ml-[9px] not-italic opacity-op5 lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_800 text-left w-[17%]"
                            compId="1083:791"
                            comWidth={67}
                            comHeight={27}
                            compLeft={14}
                            compRight={0}
                            compType="Text"
                          >{`Sold 99`}</Text>
                        </Row>
                        <Text
                          className="font-medium mr-[10px] lg:mr-[5px] xl:mr-[6px] 2xl:mr-[7px] 3xl:mr-[9px] lg:mt-[14px] xl:mt-[16px] 2xl:mt-[18px] 3xl:mt-[21px] mt-[24px] lg:text-fs14 xl:text-fs16 2xl:text-fs18 3xl:text-fs21 text-fs24 text-gray_800 text-left w-[29%]"
                          compId="1083:766"
                          comWidth={151}
                          comHeight={36}
                          compLeft={0}
                          compRight={10}
                          compType="Text"
                        >{`Smart TV HD`}</Text>
                        <Text
                          className="font-normal leading-lh xl:mt-[10px] 2xl:mt-[12px] 3xl:mt-[14px] mt-[16px] lg:mt-[9px] not-italic opacity-op5 lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_800 text-left w-[100%]"
                          compId="1083:767"
                          comWidth={512}
                          comHeight={54}
                          compLeft={0}
                          compRight={0}
                          compType="Text"
                        >{`Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore.`}</Text>
                      </Column>
                      <Column
                        className="items-start justify-start lg:mt-[44px] xl:mt-[51px] 2xl:mt-[57px] 3xl:mt-[69px] mt-[77px] w-[16%]"
                        compId="1303"
                        comWidth={122}
                        comHeight={90}
                        compLeft={0}
                        compRight={0}
                        compType="Column"
                      >
                        <Text
                          className="font-normal line-through ml-[10px] lg:ml-[5px] xl:ml-[6px] 2xl:ml-[7px] 3xl:ml-[9px] not-italic lg:text-fs14 xl:text-fs16 2xl:text-fs18 3xl:text-fs21 text-fs24 text-gray_500 text-left w-[66%]"
                          compId="1083:773"
                          comWidth={80}
                          comHeight={36}
                          compLeft={10}
                          compRight={0}
                          compType="Text"
                        >{`$39.99`}</Text>
                        <Column
                          className="w-[100%]"
                          compId="1362"
                          comWidth={122}
                          comHeight={54}
                          compLeft={0}
                          compRight={0}
                          compType="Column"
                        >
                          <Text
                            className="font-medium text-deep_orange_400 lg:text-fs21 xl:text-fs24 2xl:text-fs27 3xl:text-fs32 text-fs36 text-left w-[100%]"
                            compId="1083:772"
                            comWidth={122}
                            comHeight={54}
                            compLeft={0}
                            compRight={0}
                            compType="Text"
                          >{`$29.99`}</Text>
                        </Column>
                      </Column>
                    </Row>
                    <Row
                      className="items-center justify-start xl:mt-[10px] 2xl:mt-[11px] 3xl:mt-[13px] mt-[15px] lg:mt-[8px] w-[100%]"
                      compId="1083:774"
                      comWidth={784}
                      comHeight={60}
                      compLeft={0}
                      compRight={0}
                      compType="Row"
                    >
                      <Button
                        className="bg-deep_orange_400 font-medium lg:py-[10px] xl:py-[12px] 2xl:py-[13px] 3xl:py-[16px] py-[18.045px] rounded-radius8 text-center lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_50 w-[23%]"
                        compId="296"
                        comWidth={180}
                        comHeight={60}
                        compLeft={0}
                        compRight={0}
                        compType="Button"
                      >{`Add to Cart`}</Button>
                      <Image
                        src={"images/img_love_1.svg"}
                        className="lg:h-[14px] xl:h-[17px] 2xl:h-[19px] 3xl:h-[22px] h-[24px] xl:mb-[11px] 3xl:mb-[15px] mb-[17.5px] lg:ml-[254px] xl:ml-[291px] 2xl:ml-[327px] 3xl:ml-[393px] ml-[437px] xl:mt-[12px] 3xl:mt-[16px] mt-[18.5px] lg:my-[10px] 2xl:my-[13px] object-contain lg:w-[14px] xl:w-[16px] 2xl:w-[18px] 3xl:w-[21px] w-[24px]"
                        compId="1083:775"
                        comWidth={24}
                        comHeight={24}
                        compLeft={437}
                        compRight={0}
                        compType="Image"
                        alt="Love"
                      />
                      <Text
                        className="font-medium xl:mb-[10px] 3xl:mb-[14px] mb-[16px] lg:ml-[4px] xl:ml-[5px] 2xl:ml-[6px] 3xl:ml-[7px] ml-[8px] xl:mt-[11px] 3xl:mt-[15px] mt-[17px] 2xl:my-[12px] lg:my-[9px] lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_800 text-left w-[17%]"
                        compId="1083:777"
                        comWidth={135}
                        comHeight={27}
                        compLeft={8}
                        compRight={0}
                        compType="Text"
                      >{`Add to Wishlist`}</Text>
                    </Row>
                  </Column>
                </Row>
                <Row
                  className="items-center justify-between xl:my-[10px] 2xl:my-[12px] 3xl:my-[14px] my-[16px] lg:my-[9px] w-[100%]"
                  compId="1310"
                  comWidth={1192}
                  comHeight={243}
                  compLeft={0}
                  compRight={0}
                  compType="Row"
                >
                  <Image
                    src={"images/img_placeholder_12.svg"}
                    className="lg:h-[142px] xl:h-[163px] 2xl:h-[183px] 3xl:h-[219px] h-[243px] object-contain w-[32%]"
                    compId="1083:796"
                    comWidth={376}
                    comHeight={243}
                    compLeft={0}
                    compRight={0}
                    compType="Image"
                    alt="placeholder"
                  />
                  <Column
                    className="mb-[1px] w-[66%]"
                    compId="1309"
                    comWidth={784}
                    comHeight={242}
                    compLeft={0}
                    compRight={0}
                    compType="Column"
                  >
                    <Row
                      className="items-start justify-between w-[100%]"
                      compId="1308"
                      comWidth={784}
                      comHeight={167}
                      compLeft={0}
                      compRight={0}
                      compType="Row"
                    >
                      <Column
                        className="items-start justify-start mb-[10px] lg:mb-[5px] xl:mb-[6px] 2xl:mb-[7px] 3xl:mb-[9px] w-[65%]"
                        compId="1083:793"
                        comWidth={512}
                        comHeight={157}
                        compLeft={0}
                        compRight={0}
                        compType="Column"
                      >
                        <Row
                          className="items-center justify-start mr-[10px] lg:mr-[5px] xl:mr-[6px] 2xl:mr-[7px] 3xl:mr-[9px] w-[76%]"
                          compId="1083:808"
                          comWidth={387}
                          comHeight={27}
                          compLeft={0}
                          compRight={10}
                          compType="Row"
                        >
                          <Row
                            className="items-center justify-between w-[44%]"
                            compId="1083:809"
                            comWidth={172}
                            comHeight={27}
                            compLeft={0}
                            compRight={0}
                            compType="Row"
                          >
                            <Text
                              className="font-medium lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_800 text-left w-[16%]"
                              compId="1083:810"
                              comWidth={28}
                              comHeight={27}
                              compLeft={0}
                              compRight={0}
                              compType="Text"
                            >{`5.0`}</Text>
                            <RatingBar
                              className="my-[1px]"
                              compId="1349"
                              comWidth={136}
                              comHeight={24}
                              compLeft={0}
                              compRight={0}
                              compType="RatingBar"
                              value={5}
                              starCount={5}
                              activeColor="var(--orange_A100)"
                              size={((window.innerWidth - 15) * 24) / 1920}
                            ></RatingBar>
                          </Row>
                          <Text
                            className="font-normal xl:ml-[10px] 2xl:ml-[12px] 3xl:ml-[14px] ml-[16px] lg:ml-[9px] not-italic lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_500 text-left w-[26%]"
                            compId="1083:817"
                            comWidth={100}
                            comHeight={27}
                            compLeft={16}
                            compRight={0}
                            compType="Text"
                          >{`Review (12)`}</Text>
                          <Line
                            className="bg-gray_500 lg:h-[13px] xl:h-[15px] 2xl:h-[17px] 3xl:h-[20px] h-[22px] xl:ml-[10px] 2xl:ml-[12px] 3xl:ml-[14px] ml-[16px] lg:ml-[9px] 2xl:my-[1px] xl:my-[1px] lg:my-[1px] my-[2.5px] 3xl:my-[2px] opacity-op5 rotate-[90deg] w-[2px]"
                            compId="1083:818"
                            comWidth={2}
                            comHeight={22}
                            compLeft={16}
                            compRight={0}
                            compType="Line"
                          />
                          <Text
                            className="font-normal 2xl:ml-[10px] 3xl:ml-[12px] ml-[14px] lg:ml-[8px] xl:ml-[9px] not-italic opacity-op5 lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_800 text-left w-[17%]"
                            compId="1083:819"
                            comWidth={67}
                            comHeight={27}
                            compLeft={14}
                            compRight={0}
                            compType="Text"
                          >{`Sold 99`}</Text>
                        </Row>
                        <Text
                          className="font-medium mr-[10px] lg:mr-[5px] xl:mr-[6px] 2xl:mr-[7px] 3xl:mr-[9px] lg:mt-[14px] xl:mt-[16px] 2xl:mt-[18px] 3xl:mt-[21px] mt-[24px] lg:text-fs14 xl:text-fs16 2xl:text-fs18 3xl:text-fs21 text-fs24 text-gray_800 text-left w-[58%]"
                          compId="1083:794"
                          comWidth={297}
                          comHeight={36}
                          compLeft={0}
                          compRight={10}
                          compType="Text"
                        >{`Smart Watch New Series`}</Text>
                        <Text
                          className="font-normal leading-lh xl:mt-[10px] 2xl:mt-[12px] 3xl:mt-[14px] mt-[16px] lg:mt-[9px] not-italic opacity-op5 lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_800 text-left w-[100%]"
                          compId="1083:795"
                          comWidth={512}
                          comHeight={54}
                          compLeft={0}
                          compRight={0}
                          compType="Text"
                        >{`Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore.`}</Text>
                      </Column>
                      <Column
                        className="items-start justify-start lg:mt-[44px] xl:mt-[51px] 2xl:mt-[57px] 3xl:mt-[69px] mt-[77px] w-[16%]"
                        compId="1307"
                        comWidth={122}
                        comHeight={90}
                        compLeft={0}
                        compRight={0}
                        compType="Column"
                      >
                        <Text
                          className="font-normal line-through ml-[10px] lg:ml-[5px] xl:ml-[6px] 2xl:ml-[7px] 3xl:ml-[9px] not-italic lg:text-fs14 xl:text-fs16 2xl:text-fs18 3xl:text-fs21 text-fs24 text-gray_500 text-left w-[66%]"
                          compId="1083:801"
                          comWidth={80}
                          comHeight={36}
                          compLeft={10}
                          compRight={0}
                          compType="Text"
                        >{`$39.99`}</Text>
                        <Column
                          className="w-[100%]"
                          compId="1363"
                          comWidth={122}
                          comHeight={54}
                          compLeft={0}
                          compRight={0}
                          compType="Column"
                        >
                          <Text
                            className="font-medium text-deep_orange_400 lg:text-fs21 xl:text-fs24 2xl:text-fs27 3xl:text-fs32 text-fs36 text-left w-[100%]"
                            compId="1083:800"
                            comWidth={122}
                            comHeight={54}
                            compLeft={0}
                            compRight={0}
                            compType="Text"
                          >{`$29.99`}</Text>
                        </Column>
                      </Column>
                    </Row>
                    <Row
                      className="items-center justify-start xl:mt-[10px] 2xl:mt-[11px] 3xl:mt-[13px] mt-[15px] lg:mt-[8px] w-[100%]"
                      compId="1083:802"
                      comWidth={784}
                      comHeight={60}
                      compLeft={0}
                      compRight={0}
                      compType="Row"
                    >
                      <Button
                        className="bg-deep_orange_400 font-medium lg:py-[10px] xl:py-[12px] 2xl:py-[13px] 3xl:py-[16px] py-[18.045px] rounded-radius8 text-center lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_50 w-[23%]"
                        compId="298"
                        comWidth={180}
                        comHeight={60}
                        compLeft={0}
                        compRight={0}
                        compType="Button"
                      >{`Add to Cart`}</Button>
                      <Image
                        src={"images/img_love_1.svg"}
                        className="lg:h-[14px] xl:h-[17px] 2xl:h-[19px] 3xl:h-[22px] h-[24px] xl:mb-[11px] 3xl:mb-[15px] mb-[17.5px] lg:ml-[254px] xl:ml-[291px] 2xl:ml-[327px] 3xl:ml-[393px] ml-[437px] xl:mt-[12px] 3xl:mt-[16px] mt-[18.5px] lg:my-[10px] 2xl:my-[13px] object-contain lg:w-[14px] xl:w-[16px] 2xl:w-[18px] 3xl:w-[21px] w-[24px]"
                        compId="1083:803"
                        comWidth={24}
                        comHeight={24}
                        compLeft={437}
                        compRight={0}
                        compType="Image"
                        alt="Love"
                      />
                      <Text
                        className="font-medium xl:mb-[10px] 3xl:mb-[14px] mb-[16px] lg:ml-[4px] xl:ml-[5px] 2xl:ml-[6px] 3xl:ml-[7px] ml-[8px] xl:mt-[11px] 3xl:mt-[15px] mt-[17px] 2xl:my-[12px] lg:my-[9px] lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_800 text-left w-[17%]"
                        compId="1083:805"
                        comWidth={135}
                        comHeight={27}
                        compLeft={8}
                        compRight={0}
                        compType="Text"
                      >{`Add to Wishlist`}</Text>
                    </Row>
                  </Column>
                </Row>
                <Row
                  className="items-center justify-between xl:my-[10px] 2xl:my-[12px] 3xl:my-[14px] my-[16px] lg:my-[9px] w-[100%]"
                  compId="1314"
                  comWidth={1192}
                  comHeight={243}
                  compLeft={0}
                  compRight={0}
                  compType="Row"
                >
                  <Image
                    src={"images/img_placeholder_13.svg"}
                    className="lg:h-[142px] xl:h-[163px] 2xl:h-[183px] 3xl:h-[219px] h-[243px] object-contain w-[32%]"
                    compId="1083:824"
                    comWidth={376}
                    comHeight={243}
                    compLeft={0}
                    compRight={0}
                    compType="Image"
                    alt="placeholder"
                  />
                  <Column
                    className="mb-[1px] w-[66%]"
                    compId="1313"
                    comWidth={784}
                    comHeight={242}
                    compLeft={0}
                    compRight={0}
                    compType="Column"
                  >
                    <Row
                      className="items-start justify-between w-[100%]"
                      compId="1312"
                      comWidth={784}
                      comHeight={167}
                      compLeft={0}
                      compRight={0}
                      compType="Row"
                    >
                      <Column
                        className="items-start justify-start mb-[10px] lg:mb-[5px] xl:mb-[6px] 2xl:mb-[7px] 3xl:mb-[9px] w-[65%]"
                        compId="1083:821"
                        comWidth={512}
                        comHeight={157}
                        compLeft={0}
                        compRight={0}
                        compType="Column"
                      >
                        <Row
                          className="items-center justify-start mr-[10px] lg:mr-[5px] xl:mr-[6px] 2xl:mr-[7px] 3xl:mr-[9px] w-[76%]"
                          compId="1083:836"
                          comWidth={387}
                          comHeight={27}
                          compLeft={0}
                          compRight={10}
                          compType="Row"
                        >
                          <Row
                            className="items-center justify-between w-[44%]"
                            compId="1083:837"
                            comWidth={172}
                            comHeight={27}
                            compLeft={0}
                            compRight={0}
                            compType="Row"
                          >
                            <Text
                              className="font-medium lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_800 text-left w-[16%]"
                              compId="1083:838"
                              comWidth={28}
                              comHeight={27}
                              compLeft={0}
                              compRight={0}
                              compType="Text"
                            >{`5.0`}</Text>
                            <RatingBar
                              className="my-[1px]"
                              compId="1350"
                              comWidth={136}
                              comHeight={24}
                              compLeft={0}
                              compRight={0}
                              compType="RatingBar"
                              value={5}
                              starCount={5}
                              activeColor="var(--orange_A100)"
                              size={((window.innerWidth - 15) * 24) / 1920}
                            ></RatingBar>
                          </Row>
                          <Text
                            className="font-normal xl:ml-[10px] 2xl:ml-[12px] 3xl:ml-[14px] ml-[16px] lg:ml-[9px] not-italic lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_500 text-left w-[26%]"
                            compId="1083:845"
                            comWidth={100}
                            comHeight={27}
                            compLeft={16}
                            compRight={0}
                            compType="Text"
                          >{`Review (12)`}</Text>
                          <Line
                            className="bg-gray_500 lg:h-[13px] xl:h-[15px] 2xl:h-[17px] 3xl:h-[20px] h-[22px] xl:ml-[10px] 2xl:ml-[12px] 3xl:ml-[14px] ml-[16px] lg:ml-[9px] 2xl:my-[1px] xl:my-[1px] lg:my-[1px] my-[2.5px] 3xl:my-[2px] opacity-op5 rotate-[90deg] w-[2px]"
                            compId="1083:846"
                            comWidth={2}
                            comHeight={22}
                            compLeft={16}
                            compRight={0}
                            compType="Line"
                          />
                          <Text
                            className="font-normal 2xl:ml-[10px] 3xl:ml-[12px] ml-[14px] lg:ml-[8px] xl:ml-[9px] not-italic opacity-op5 lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_800 text-left w-[17%]"
                            compId="1083:847"
                            comWidth={67}
                            comHeight={27}
                            compLeft={14}
                            compRight={0}
                            compType="Text"
                          >{`Sold 99`}</Text>
                        </Row>
                        <Text
                          className="font-medium mr-[10px] lg:mr-[5px] xl:mr-[6px] 2xl:mr-[7px] 3xl:mr-[9px] lg:mt-[14px] xl:mt-[16px] 2xl:mt-[18px] 3xl:mt-[21px] mt-[24px] lg:text-fs14 xl:text-fs16 2xl:text-fs18 3xl:text-fs21 text-fs24 text-gray_800 text-left w-[44%]"
                          compId="1083:822"
                          comWidth={224}
                          comHeight={36}
                          compLeft={0}
                          compRight={10}
                          compType="Text"
                        >{`Camera HD + Lens`}</Text>
                        <Text
                          className="font-normal leading-lh xl:mt-[10px] 2xl:mt-[12px] 3xl:mt-[14px] mt-[16px] lg:mt-[9px] not-italic opacity-op5 lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_800 text-left w-[100%]"
                          compId="1083:823"
                          comWidth={512}
                          comHeight={54}
                          compLeft={0}
                          compRight={0}
                          compType="Text"
                        >{`Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore.`}</Text>
                      </Column>
                      <Column
                        className="items-start justify-start lg:mt-[44px] xl:mt-[51px] 2xl:mt-[57px] 3xl:mt-[69px] mt-[77px] w-[16%]"
                        compId="1311"
                        comWidth={122}
                        comHeight={90}
                        compLeft={0}
                        compRight={0}
                        compType="Column"
                      >
                        <Text
                          className="font-normal line-through ml-[10px] lg:ml-[5px] xl:ml-[6px] 2xl:ml-[7px] 3xl:ml-[9px] not-italic lg:text-fs14 xl:text-fs16 2xl:text-fs18 3xl:text-fs21 text-fs24 text-gray_500 text-left w-[66%]"
                          compId="1083:829"
                          comWidth={80}
                          comHeight={36}
                          compLeft={10}
                          compRight={0}
                          compType="Text"
                        >{`$39.99`}</Text>
                        <Column
                          className="w-[100%]"
                          compId="1364"
                          comWidth={122}
                          comHeight={54}
                          compLeft={0}
                          compRight={0}
                          compType="Column"
                        >
                          <Text
                            className="font-medium text-deep_orange_400 lg:text-fs21 xl:text-fs24 2xl:text-fs27 3xl:text-fs32 text-fs36 text-left w-[100%]"
                            compId="1083:828"
                            comWidth={122}
                            comHeight={54}
                            compLeft={0}
                            compRight={0}
                            compType="Text"
                          >{`$29.99`}</Text>
                        </Column>
                      </Column>
                    </Row>
                    <Row
                      className="items-center justify-start xl:mt-[10px] 2xl:mt-[11px] 3xl:mt-[13px] mt-[15px] lg:mt-[8px] w-[100%]"
                      compId="1083:830"
                      comWidth={784}
                      comHeight={60}
                      compLeft={0}
                      compRight={0}
                      compType="Row"
                    >
                      <Button
                        className="bg-deep_orange_400 font-medium lg:py-[10px] xl:py-[12px] 2xl:py-[13px] 3xl:py-[16px] py-[18.045px] rounded-radius8 text-center lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_50 w-[23%]"
                        compId="300"
                        comWidth={180}
                        comHeight={60}
                        compLeft={0}
                        compRight={0}
                        compType="Button"
                      >{`Add to Cart`}</Button>
                      <Image
                        src={"images/img_love_1.svg"}
                        className="lg:h-[14px] xl:h-[17px] 2xl:h-[19px] 3xl:h-[22px] h-[24px] xl:mb-[11px] 3xl:mb-[15px] mb-[17.5px] lg:ml-[254px] xl:ml-[291px] 2xl:ml-[327px] 3xl:ml-[393px] ml-[437px] xl:mt-[12px] 3xl:mt-[16px] mt-[18.5px] lg:my-[10px] 2xl:my-[13px] object-contain lg:w-[14px] xl:w-[16px] 2xl:w-[18px] 3xl:w-[21px] w-[24px]"
                        compId="1083:831"
                        comWidth={24}
                        comHeight={24}
                        compLeft={437}
                        compRight={0}
                        compType="Image"
                        alt="Love"
                      />
                      <Text
                        className="font-medium xl:mb-[10px] 3xl:mb-[14px] mb-[16px] lg:ml-[4px] xl:ml-[5px] 2xl:ml-[6px] 3xl:ml-[7px] ml-[8px] xl:mt-[11px] 3xl:mt-[15px] mt-[17px] 2xl:my-[12px] lg:my-[9px] lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_800 text-left w-[17%]"
                        compId="1083:833"
                        comWidth={135}
                        comHeight={27}
                        compLeft={8}
                        compRight={0}
                        compType="Text"
                      >{`Add to Wishlist`}</Text>
                    </Row>
                  </Column>
                </Row>
              </List>
              <Row
                className="items-center justify-center lg:mt-[46px] xl:mt-[53px] 2xl:mt-[60px] 3xl:mt-[72px] mt-[80px] mx-[auto] rounded-radius8 w-[25%]"
                compId="1315"
                comWidth={296}
                comHeight={51}
                compLeft={10}
                compRight={10}
                compType="Row"
              >
                <Image
                  src={"images/img_chevrondown_4.svg"}
                  className="lg:h-[14px] xl:h-[17px] 2xl:h-[19px] 3xl:h-[22px] h-[24px] 2xl:my-[10px] 3xl:my-[12px] my-[13.5px] lg:my-[7px] xl:my-[9px] object-contain lg:w-[14px] xl:w-[16px] 2xl:w-[18px] 3xl:w-[21px] w-[24px]"
                  compId="1083:919"
                  comWidth={24}
                  comHeight={24}
                  compLeft={0}
                  compRight={0}
                  compType="Image"
                  alt="chevrondown"
                />
                <Text
                  className="bg-deep_orange_400 font-normal lg:h-[30px] xl:h-[34px] 2xl:h-[38px] 3xl:h-[46px] h-[50px] leading-lh lg:ml-[14px] xl:ml-[16px] 2xl:ml-[18px] 3xl:ml-[21px] ml-[24px] my-[1px] not-italic 3xl:pb-[10px] pb-[12px] lg:pb-[7px] xl:pb-[8px] 2xl:pb-[9px] lg:pl-[12px] xl:pl-[14px] 2xl:pl-[16px] 3xl:pl-[19px] pl-[22px] pt-[11px] lg:pt-[6px] xl:pt-[7px] 2xl:pt-[8px] 3xl:pt-[9px] rounded-radius8 lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_50 text-left lg:w-[29px] xl:w-[33px] 2xl:w-[37px] 3xl:w-[45px] w-[50px]"
                  compId="1083:921"
                  comWidth={50}
                  comHeight={50}
                  compLeft={24}
                  compRight={0}
                  compType="Text"
                >{`1`}</Text>
                <Text
                  className="border-2 border-gray_500 border-solid font-normal lg:h-[30px] xl:h-[35px] 2xl:h-[39px] 3xl:h-[46px] h-[51px] leading-lh lg:ml-[14px] xl:ml-[16px] 2xl:ml-[18px] 3xl:ml-[21px] ml-[24px] not-italic lg:pl-[11px] xl:pl-[13px] 2xl:pl-[15px] 3xl:pl-[18px] pl-[20px] 3xl:py-[10px] py-[12px] lg:py-[7px] xl:py-[8px] 2xl:py-[9px] rounded-radius8 lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_500 text-left lg:w-[29px] xl:w-[34px] 2xl:w-[38px] 3xl:w-[45px] w-[51px]"
                  compId="1083:924"
                  comWidth={51}
                  comHeight={51}
                  compLeft={24}
                  compRight={0}
                  compType="Text"
                >{`2`}</Text>
                <Text
                  className="border-2 border-gray_500 border-solid font-normal lg:h-[30px] xl:h-[35px] 2xl:h-[39px] 3xl:h-[46px] h-[51px] leading-lh lg:ml-[14px] xl:ml-[16px] 2xl:ml-[18px] 3xl:ml-[21px] ml-[24px] not-italic lg:pl-[11px] xl:pl-[13px] 2xl:pl-[15px] 3xl:pl-[18px] pl-[20px] 3xl:py-[10px] py-[12px] lg:py-[7px] xl:py-[8px] 2xl:py-[9px] rounded-radius8 lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_500 text-left lg:w-[29px] xl:w-[34px] 2xl:w-[38px] 3xl:w-[45px] w-[51px]"
                  compId="1083:927"
                  comWidth={51}
                  comHeight={51}
                  compLeft={24}
                  compRight={0}
                  compType="Text"
                >{`3`}</Text>
                <Image
                  src={"images/img_chevrondown_5.svg"}
                  className="lg:h-[14px] xl:h-[17px] 2xl:h-[19px] 3xl:h-[22px] h-[24px] lg:ml-[14px] xl:ml-[16px] 2xl:ml-[18px] 3xl:ml-[21px] ml-[24px] 2xl:my-[10px] 3xl:my-[12px] my-[13.5px] lg:my-[7px] xl:my-[9px] object-contain lg:w-[14px] xl:w-[16px] 2xl:w-[18px] 3xl:w-[21px] w-[24px]"
                  compId="1083:930"
                  comWidth={24}
                  comHeight={24}
                  compLeft={24}
                  compRight={0}
                  compType="Image"
                  alt="chevrondown"
                />
              </Row>
            </Column>
          </Row>
          <Row
            className="bg-deep_orange_400 font-poppins items-center justify-start xl:mt-[106px] 2xl:mt-[120px] 3xl:mt-[144px] mt-[160px] lg:mt-[93px] w-[100%]"
            compId="1316"
            comWidth={1920}
            comHeight={245}
            compLeft={0}
            compRight={0}
            compType="Row"
          >
            <Image
              src={"images/img_newsletter_1.svg"}
              className="lg:h-[56px] xl:h-[65px] 2xl:h-[73px] 3xl:h-[87px] h-[96px] xl:mb-[49px] 2xl:mb-[55px] 3xl:mb-[66px] mb-[74px] lg:ml-[149px] xl:ml-[170px] 2xl:ml-[192px] 3xl:ml-[230px] ml-[256px] xl:mt-[50px] 2xl:mt-[56px] 3xl:mt-[67px] mt-[75px] lg:my-[43px] object-contain lg:w-[56px] xl:w-[64px] 2xl:w-[72px] 3xl:w-[86px] w-[96px]"
              compId="1083:851"
              comWidth={96}
              comHeight={96}
              compLeft={256}
              compRight={0}
              compType="Image"
              alt="Newsletter"
            />
            <Column
              className="items-start lg:ml-[18px] xl:ml-[21px] 2xl:ml-[24px] 3xl:ml-[28px] ml-[32px] lg:my-[40px] xl:my-[46px] 2xl:my-[51px] 3xl:my-[62px] my-[69px] w-[33%]"
              compId="1083:859"
              comWidth={637}
              comHeight={107}
              compLeft={32}
              compRight={0}
              compType="Column"
            >
              <Text
                className="font-bold font-montserrat lg:text-fs28 xl:text-fs32 2xl:text-fs36 3xl:text-fs43 text-fs48 text-gray_50 text-left w-[100%]"
                compId="1083:860"
                comWidth={637}
                comHeight={59}
                compLeft={0}
                compRight={0}
                compType="Text"
              >{`Join our newsletter now!`}</Text>
              <Text
                className="font-normal font-poppins mr-[10px] lg:mr-[5px] xl:mr-[6px] 2xl:mr-[7px] 3xl:mr-[9px] xl:mt-[10px] 2xl:mt-[12px] 3xl:mt-[14px] mt-[16px] lg:mt-[9px] not-italic lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_50 text-left w-[75%]"
                compId="1083:861"
                comWidth={480}
                comHeight={32}
                compLeft={0}
                compRight={10}
                compType="Text"
              >{`Register now and get our latest updates and promos.`}</Text>
            </Column>
            <Stack
              className="lg:h-[45px] xl:h-[51px] 2xl:h-[58px] 3xl:h-[69px] h-[76px] 3xl:mb-[75px] mb-[84px] 3xl:ml-[105px] ml-[117px] lg:ml-[68px] xl:ml-[78px] 2xl:ml-[87px] 3xl:mt-[76px] mt-[85px] lg:my-[49px] xl:my-[56px] 2xl:my-[63px] w-[27%]"
              compId="1341"
              comWidth={526}
              comHeight={76}
              compLeft={117}
              compRight={0}
              compType="Stack"
            >
              <Input
                className="absolute bg-gray_50 placeholder:bg-transparent border-0 font-normal inset-[0] not-italic lg:pb-[15px] pb-[26.545px] lg:pl-[18px] xl:pl-[21px] 2xl:pl-[24px] 3xl:pl-[28px] pl-[32px] lg:pt-[14px] pt-[25.545px] xl:py-[17px] 2xl:py-[19px] 3xl:py-[23px] rounded-radius8 lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 placeholder:text-gray_500 text-gray_500 text-left tracking-ls1 w-[100%]"
                compId="305"
                comWidth={526}
                comHeight={76}
                compLeft={0}
                compRight={0}
                compType="EditText"
                name="Group305"
                placeholder={`Enter your email`}
              ></Input>
              <Button
                className="absolute bg-gray_800 font-bold h-[max-content] inset-y-[0] my-[auto] lg:py-[10px] xl:py-[12px] 2xl:py-[13px] 3xl:py-[16px] py-[18.045px] right-[2%] rounded-radius8 text-center lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_50 w-[20%]"
                compId="301"
                comWidth={105}
                comHeight={60}
                compLeft={0}
                compRight={8}
                compType="Button"
              >{`Join`}</Button>
            </Stack>
          </Row>
          <footer
            className="font-poppins w-[100%]"
            compId="1083:862"
            comWidth={1920}
            comHeight={585}
            compLeft={0}
            compRight={0}
            compType="Footer"
          >
            <Column
              className="bg-deep_orange_400_cf items-center justify-end w-[100%]"
              compId="1320"
              comWidth={1920}
              comHeight={585}
              compLeft={0}
              compRight={0}
              compType="Column"
            >
              <Row
                className="items-start justify-start lg:mt-[42px] xl:mt-[48px] 2xl:mt-[54px] 3xl:mt-[65px] mt-[73px] w-[100%]"
                compId="1317"
                comWidth={1920}
                comHeight={260}
                compLeft={0}
                compRight={0}
                compType="Row"
              >
                <Column
                  className="items-start justify-start 2xl:mb-[10px] 3xl:mb-[12px] mb-[14px] lg:mb-[8px] xl:mb-[9px] xl:ml-[106px] 2xl:ml-[120px] 3xl:ml-[144px] ml-[160px] lg:ml-[93px] w-[20%]"
                  compId="1319"
                  comWidth={381}
                  comHeight={246}
                  compLeft={160}
                  compRight={0}
                  compType="Column"
                >
                  <Text
                    className="font-semibold mr-[10px] lg:mr-[5px] xl:mr-[6px] 2xl:mr-[7px] 3xl:mr-[9px] text-deep_orange_400 lg:text-fs21 xl:text-fs24 2xl:text-fs27 3xl:text-fs32 text-fs36 text-left w-[29%]"
                    compId="1083:873"
                    comWidth={112}
                    comHeight={54}
                    compLeft={0}
                    compRight={10}
                    compType="Text"
                  >{`Tronix`}</Text>
                  <Text
                    className="font-normal lg:leading-lh18 xl:leading-lh21 2xl:leading-lh24 3xl:leading-lh28 leading-lh3200 xl:mt-[10px] 2xl:mt-[12px] 3xl:mt-[14px] mt-[16px] lg:mt-[9px] not-italic opacity-op8 lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_800 text-left w-[100%]"
                    compId="1083:874"
                    comWidth={381}
                    comHeight={64}
                    compLeft={0}
                    compRight={0}
                    compType="Text"
                  >{`Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor.`}</Text>
                  <Row
                    className="items-center justify-start mr-[10px] lg:mr-[5px] xl:mr-[6px] 2xl:mr-[7px] 3xl:mr-[9px] lg:mt-[18px] xl:mt-[21px] 2xl:mt-[24px] 3xl:mt-[28px] mt-[32px] w-[64%]"
                    compId="1083:909"
                    comWidth={243}
                    comHeight={32}
                    compLeft={0}
                    compRight={10}
                    compType="Row"
                  >
                    <Image
                      src={"images/img_call_2.svg"}
                      className="lg:h-[14px] xl:h-[17px] 2xl:h-[19px] 3xl:h-[22px] h-[24px] xl:my-[2px] lg:my-[2px] 3xl:my-[3px] 2xl:my-[3px] my-[4px] object-contain lg:w-[14px] xl:w-[16px] 2xl:w-[18px] 3xl:w-[21px] w-[24px]"
                      compId="1083:910"
                      comWidth={24}
                      comHeight={24}
                      compLeft={0}
                      compRight={0}
                      compType="Image"
                      alt="Call"
                    />
                    <Text
                      className="font-normal lg:ml-[4px] xl:ml-[5px] 2xl:ml-[6px] 3xl:ml-[7px] ml-[8px] not-italic lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_800 text-left w-[87%]"
                      compId="1083:912"
                      comWidth={211}
                      comHeight={32}
                      compLeft={8}
                      compRight={0}
                      compType="Text"
                    >{`+1234567890`}</Text>
                  </Row>
                  <Row
                    className="items-center justify-start mr-[10px] lg:mr-[5px] xl:mr-[6px] 2xl:mr-[7px] 3xl:mr-[9px] xl:mt-[10px] 2xl:mt-[12px] 3xl:mt-[14px] mt-[16px] lg:mt-[9px] w-[64%]"
                    compId="1083:913"
                    comWidth={243}
                    comHeight={32}
                    compLeft={0}
                    compRight={10}
                    compType="Row"
                  >
                    <Image
                      src={"images/img_email_2.svg"}
                      className="lg:h-[14px] xl:h-[17px] 2xl:h-[19px] 3xl:h-[22px] h-[24px] xl:my-[2px] lg:my-[2px] 3xl:my-[3px] 2xl:my-[3px] my-[4px] object-contain lg:w-[14px] xl:w-[16px] 2xl:w-[18px] 3xl:w-[21px] w-[24px]"
                      compId="1083:914"
                      comWidth={24}
                      comHeight={24}
                      compLeft={0}
                      compRight={0}
                      compType="Image"
                      alt="Email"
                    />
                    <Text
                      className="font-normal lg:ml-[4px] xl:ml-[5px] 2xl:ml-[6px] 3xl:ml-[7px] ml-[8px] not-italic lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_800 text-left w-[87%]"
                      compId="1083:916"
                      comWidth={211}
                      comHeight={32}
                      compLeft={8}
                      compRight={0}
                      compType="Text"
                    >{`lovia@support.com`}</Text>
                  </Row>
                </Column>
                <Column
                  className="font-montserrat items-start justify-start lg:ml-[253px] xl:ml-[290px] 2xl:ml-[326px] 3xl:ml-[391px] ml-[435px] w-[6%]"
                  compId="1083:875"
                  comWidth={120}
                  comHeight={260}
                  compLeft={435}
                  compRight={0}
                  compType="Column"
                >
                  <Text
                    className="font-bold lg:text-fs14 xl:text-fs16 2xl:text-fs18 3xl:text-fs21 text-fs24 text-gray_800 text-left w-[100%]"
                    compId="1083:876"
                    comWidth={120}
                    comHeight={29}
                    compLeft={0}
                    compRight={0}
                    compType="Text"
                  >{`Company`}</Text>
                  <Column
                    className="font-poppins items-start justify-start mr-[10px] lg:mr-[5px] xl:mr-[6px] 2xl:mr-[7px] 3xl:mr-[9px] lg:mt-[18px] xl:mt-[21px] 2xl:mt-[24px] 3xl:mt-[28px] mt-[32px] w-[67%]"
                    compId="1083:877"
                    comWidth={80}
                    comHeight={199}
                    compLeft={0}
                    compRight={10}
                    compType="Column"
                  >
                    <Text
                      className="font-normal mr-[10px] lg:mr-[5px] xl:mr-[6px] 2xl:mr-[7px] 3xl:mr-[9px] not-italic opacity-op8 lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_800 text-left w-[68%]"
                      compId="1083:878"
                      comWidth={54}
                      comHeight={27}
                      compLeft={0}
                      compRight={10}
                      compType="Text"
                    >{`About`}</Text>
                    <Text
                      className="font-normal xl:mt-[10px] 2xl:mt-[12px] 3xl:mt-[14px] mt-[16px] lg:mt-[9px] not-italic opacity-op8 lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_800 text-left w-[100%]"
                      compId="1083:879"
                      comWidth={80}
                      comHeight={27}
                      compLeft={0}
                      compRight={0}
                      compType="Text"
                    >{`Products`}</Text>
                    <Text
                      className="font-normal lg:mr-[3px] 2xl:mr-[4px] xl:mr-[4px] 3xl:mr-[5px] mr-[6px] xl:mt-[10px] 2xl:mt-[12px] 3xl:mt-[14px] mt-[16px] lg:mt-[9px] not-italic opacity-op8 lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_800 text-left w-[93%]"
                      compId="1083:880"
                      comWidth={74}
                      comHeight={27}
                      compLeft={0}
                      compRight={6}
                      compType="Text"
                    >{`Contact`}</Text>
                    <Text
                      className="font-normal mr-[10px] lg:mr-[5px] xl:mr-[6px] 2xl:mr-[7px] 3xl:mr-[9px] xl:mt-[10px] 2xl:mt-[12px] 3xl:mt-[14px] mt-[16px] lg:mt-[9px] not-italic opacity-op8 lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_800 text-left w-[50%]"
                      compId="1083:881"
                      comWidth={40}
                      comHeight={27}
                      compLeft={0}
                      compRight={10}
                      compType="Text"
                    >{`Blog`}</Text>
                    <Text
                      className="font-normal lg:mr-[4px] xl:mr-[5px] 2xl:mr-[6px] 3xl:mr-[7px] mr-[8px] xl:mt-[10px] 2xl:mt-[12px] 3xl:mt-[14px] mt-[16px] lg:mt-[9px] not-italic opacity-op8 lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_800 text-left w-[90%]"
                      compId="1083:882"
                      comWidth={72}
                      comHeight={27}
                      compLeft={0}
                      compRight={8}
                      compType="Text"
                    >{`Careers`}</Text>
                  </Column>
                </Column>
                <Column
                  className="font-montserrat items-start justify-start lg:mb-[25px] xl:mb-[28px] 2xl:mb-[32px] 3xl:mb-[38px] mb-[43px] xl:ml-[101px] 2xl:ml-[114px] 3xl:ml-[136px] ml-[152px] lg:ml-[88px] w-[9%]"
                  compId="1083:883"
                  comWidth={164}
                  comHeight={217}
                  compLeft={152}
                  compRight={0}
                  compType="Column"
                >
                  <Text
                    className="font-bold mr-[10px] lg:mr-[5px] xl:mr-[6px] 2xl:mr-[7px] 3xl:mr-[9px] lg:text-fs14 xl:text-fs16 2xl:text-fs18 3xl:text-fs21 text-fs24 text-gray_800 text-left w-[91%]"
                    compId="1083:884"
                    comWidth={150}
                    comHeight={29}
                    compLeft={0}
                    compRight={10}
                    compType="Text"
                  >{`Information`}</Text>
                  <Column
                    className="font-poppins lg:mt-[18px] xl:mt-[21px] 2xl:mt-[24px] 3xl:mt-[28px] mt-[32px] w-[100%]"
                    compId="1365"
                    comWidth={164}
                    comHeight={156}
                    compLeft={0}
                    compRight={0}
                    compType="Column"
                  >
                    <Column
                      className="items-start justify-start w-[100%]"
                      compId="1083:885"
                      comWidth={164}
                      comHeight={156}
                      compLeft={0}
                      compRight={0}
                      compType="Column"
                    >
                      <Text
                        className="font-normal mr-[10px] lg:mr-[5px] xl:mr-[6px] 2xl:mr-[7px] 3xl:mr-[9px] not-italic opacity-op8 lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_800 text-left w-[65%]"
                        compId="1083:886"
                        comWidth={107}
                        comHeight={27}
                        compLeft={0}
                        compRight={10}
                        compType="Text"
                      >{`Help Center`}</Text>
                      <Text
                        className="font-normal xl:mt-[10px] 2xl:mt-[12px] 3xl:mt-[14px] mt-[16px] lg:mt-[9px] not-italic opacity-op8 lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_800 text-left w-[100%]"
                        compId="1083:887"
                        comWidth={164}
                        comHeight={27}
                        compLeft={0}
                        compRight={0}
                        compType="Text"
                      >{`Payment Methods`}</Text>
                      <Text
                        className="font-normal mr-[10px] lg:mr-[5px] xl:mr-[6px] 2xl:mr-[7px] 3xl:mr-[9px] xl:mt-[10px] 2xl:mt-[12px] 3xl:mt-[14px] mt-[16px] lg:mt-[9px] not-italic opacity-op8 lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_800 text-left w-[88%]"
                        compId="1083:888"
                        comWidth={145}
                        comHeight={27}
                        compLeft={0}
                        compRight={10}
                        compType="Text"
                      >{`Return & Refund`}</Text>
                      <Text
                        className="font-normal mr-[10px] lg:mr-[5px] xl:mr-[6px] 2xl:mr-[7px] 3xl:mr-[9px] xl:mt-[10px] 2xl:mt-[12px] 3xl:mt-[14px] mt-[16px] lg:mt-[9px] not-italic opacity-op8 lg:text-fs10 xl:text-fs12 2xl:text-fs13 3xl:text-fs16 text-fs18 text-gray_800 text-left w-[74%]"
                        compId="1083:889"
                        comWidth={122}
                        comHeight={27}
                        compLeft={0}
                        compRight={10}
                        compType="Text"
                      >{`Privacy Policy`}</Text>
                    </Column>
                  </Column>
                </Column>
                <Column
                  className="font-montserrat items-start justify-start 2xl:mb-[107px] 3xl:mb-[128px] mb-[143px] lg:mb-[83px] xl:mb-[95px] ml-[108px] lg:ml-[63px] xl:ml-[72px] 2xl:ml-[81px] 3xl:ml-[97px] w-[11%]"
                  compId="1083:890"
                  comWidth={216}
                  comHeight={117}
                  compLeft={108}
                  compRight={0}
                  compType="Column"
                >
                  <Text
                    className="font-bold mr-[10px] lg:mr-[5px] xl:mr-[6px] 2xl:mr-[7px] 3xl:mr-[9px] lg:text-fs14 xl:text-fs16 2xl:text-fs18 3xl:text-fs21 text-fs24 text-gray_800 text-left w-[56%]"
                    compId="1083:891"
                    comWidth={122}
                    comHeight={29}
                    compLeft={0}
                    compRight={10}
                    compType="Text"
                  >{`Follow Us`}</Text>
                  <Column
                    className="lg:mt-[18px] xl:mt-[21px] 2xl:mt-[24px] 3xl:mt-[28px] mt-[32px] w-[100%]"
                    compId="1366"
                    comWidth={216}
                    comHeight={56}
                    compLeft={0}
                    compRight={0}
                    compType="Column"
                  >
                    <Image
                      src={"images/img_sosmedia_12.svg"}
                      className="lg:h-[33px] xl:h-[38px] 2xl:h-[43px] 3xl:h-[51px] h-[56px] object-cover w-[100%]"
                      compId="1083:892"
                      comWidth={216}
                      comHeight={56}
                      compLeft={0}
                      compRight={0}
                      compType="Image"
                      alt="sosmedia"
                    />
                  </Column>
                </Column>
              </Row>
              <Line
                className="bg-gray_500 h-[1px] 3xl:mt-[114px] mt-[127px] lg:mt-[74px] xl:mt-[84px] 2xl:mt-[95px] mx-[auto] w-[83%]"
                compId="1083:917"
                comWidth={1600}
                comHeight={1}
                compLeft={10}
                compRight={10}
                compType="Line"
              />
              <Row
                className="items-center justify-between lg:mb-[31px] xl:mb-[36px] 2xl:mb-[40px] 3xl:mb-[48px] mb-[54px] lg:mt-[22px] xl:mt-[25px] 2xl:mt-[28px] 3xl:mt-[34px] mt-[38px] w-[100%]"
                compId="1318"
                comWidth={1920}
                comHeight={32}
                compLeft={0}
                compRight={0}
                compType="Row"
              >
                <Text
                  className="font-normal lg:mb-[3px] 2xl:mb-[4px] xl:mb-[4px] 3xl:mb-[5px] mb-[6px] xl:ml-[106px] 2xl:ml-[120px] 3xl:ml-[144px] ml-[160px] lg:ml-[93px] lg:mt-[2px] 2xl:mt-[3px] xl:mt-[3px] 3xl:mt-[4px] mt-[5px] not-italic 2xl:text-fs10 3xl:text-fs12 text-fs14 lg:text-fs8 xl:text-fs9 text-gray_800 text-left w-[15%]"
                  compId="1083:871"
                  comWidth={287}
                  comHeight={21}
                  compLeft={160}
                  compRight={0}
                  compType="Text"
                >{`Copyright © 2021 Tronix. All Right Reseved`}</Text>
                <Image
                  src={"images/img_payment_4.svg"}
                  className="lg:h-[19px] xl:h-[22px] 2xl:h-[25px] 3xl:h-[29px] h-[32px] xl:mr-[106px] 2xl:mr-[120px] 3xl:mr-[144px] mr-[160px] lg:mr-[93px] object-contain w-[7%]"
                  compId="1083:864"
                  comWidth={128}
                  comHeight={32}
                  compLeft={0}
                  compRight={160}
                  compType="Image"
                  alt="payment"
                />
              </Row>
            </Column>
          </footer>
        </Column>
      </Column>
    </>
  );
};

export default SearchPagePage;
